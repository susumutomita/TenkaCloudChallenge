...
describe("sha256-schedule-logic: participant/verifier separation (Issue 537/538)", () => {
  it("keeps fixtures/, the derivation and the hidden suite out of the participant Docker stage", () => {
    ...
    // to the participant stage put `rotate_case`/`mux_case` in the same container a
    // learner's own build produced, and `verifier/server.py` shipping there put
    // `first_affected_index` right alongside it -- no matter where the comparison
    // itself lived.
  ...
  it("reproduces the original leak: rotate_case/mux_case/first_affected_index are no longer in any file the participant image carries", () => {
    ...
    // so a COPY that puts `fixtures/` or `verifier/` back fails this test.
    const participantFiles = participantPythonFiles(
      join(import.meta.dir, ".."),
      "challenges/sha256-schedule-logic",
    );
    expect(participantFiles).not.toContain(
      "challenges/sha256-schedule-logic/local/fixtures/generate.py",
    );
    expect(participantFiles).not.toContain(
      "challenges/sha256-schedule-logic/local/verifier/server.py",
    );
    expect(participantFiles).toContain(
      "challenges/sha256-schedule-logic/local/tests/public/test_schedule.py",
    );
    for (const file of participantFiles) {
      ...
      expect(source).not.toContain("first_affected_index(");
    }
  });
  ...
  it("re-checks the answer seal in the verifier, so bypassing the Workbench does not credit a bare answer", () => {
    const script = [
      ...
      "print(json.dumps({",
      "  'bare': _unwrap_submission('rotate', 'deadbeef,deadbeef'),",
      "  'forged': _unwrap_submission('dependency', 'tcw1.eyJ2IjoxfQ.AAAA'),",
      "  'code': _unwrap_submission('sigma', 'def rotr(): pass'),",
      "}))",
    ].join("\n");
    ...
    expect(output.code).toBe("def rotr(): pass");
  });

  it("accepts a sealed dependency answer end to end, Workbench seal through verifier unwrap", () => {
    const script = [
      "import json, sys",
      "sys.path.insert(0, '.')",
      "from participant.server import _WORKBENCH",
      "from verifier.server import _unwrap_submission, evaluate",
      "prepared = _WORKBENCH.prepare_submissions(",
      "    _WORKBENCH.starter_payload(),",
      "    {'rotate': '', 'mux': '', 'dependency': json.dumps(16)},",
      ")",
      "sealed = prepared['submissions']['dependency']",
      "unwrapped = _unwrap_submission('dependency', sealed)",
      "print(json.dumps({'sealed': sealed.startswith('tcw1.'), 'unwrapped': unwrapped}))",
    ].join("\n");
    const result = python(["-c", script]);
    expect(result.status).toBe(0);
    const output = JSON.parse(result.stdout.trim().split("\n").at(-1) ?? "{}") as {
    ...
    expect(output.unwrapped).toBe(16);
  });
...
`;
    ...
    const expectedVerdicts = ["rotate", "mux", "dependency", "sigma", "logic", "schedule"].map(
  ...
  it("serves the public evidence without the derivation the checkpoints are graded on", () => {
    const script = [
      "import json, sys",
      "sys.path.insert(0, '.')",
    ...
    expect(output.keys).toEqual(["dependency", "healthToken", "mux", "rotate"]);
  });
});
