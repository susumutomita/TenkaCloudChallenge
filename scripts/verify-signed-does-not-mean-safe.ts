...
import { inspectStarter, STARTER_TRIAGE } from "../challenges/signed-does-not-mean-safe/local/app/engine.mjs";
// @ts-expect-error The hidden verifier is intentionally plain JavaScript.
import { CHECKPOINT_IDS } from "../challenges/signed-does-not-mean-safe/local/verifier/grader.mjs";
...
const LOCAL_DIR = join(REPO_ROOT, "challenges/signed-does-not-mean-safe/local");
const project = `tcc-signed-npm-${process.pid}`;
...
const environment = {
  ...process.env,
  SIGNED_NPM_WORKBENCH_PORT: String(workbenchPort),
  SIGNED_NPM_VERIFY_PORT: String(verifyPort),
  SIGNED_NPM_PARTICIPANT_IMAGE: participantImage,
  SIGNED_NPM_VERIFIER_IMAGE: verifierImage,
};
...
try {
  ...
  run("docker", [
    ...
    participantImage,
    "-c",
    "test ! -e /app/verifier && test ! -e /app/reference.mjs && test ! -e /app/mutation.mjs",
  ]);
  ...
  if (!home.includes("署名済みなら、安全？")) throw new Error("Browser Workbench did not render");
  ...
  if (
    ...
    !observation.cases.every((item: { expected?: string; actual?: string }) => item.expected !== item.actual)
  ) {
    throw new Error(`starter did not expose all four blind spots: ${JSON.stringify(observation)}`);
  }
  ...
  const oversized = await post("/verify", { checkpointId: "artifact-inventory", submission: "a".repeat(17 * 1024) });
  ...
  compose(["exec", "-T", "participant", "sh", "-c", "test ! -w /app && test ! -e /app/verifier && test ! -e /app/reference.mjs"]);
...
console.log("signed-does-not-mean-safe runtime proof passed and cleaned all dedicated resources");
