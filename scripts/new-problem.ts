...
function main(): void {
  ...
  console.log(
    [
      ...
      `  3. bun run check:problem ${id}   # the shipping gate for your problem alone`,
      "     (schema, participant-visible surface, local-play shape, static solvability)",
      "  4. bun run validate   # the whole catalog contract, once your problem is green",
      `  5. If the problem is local-play: bun run scripts/solvability-audit.ts --problem ${id}`,
      "     runs the full per-problem sweep — is every checkpoint actually answerable?",
      "  6. Set status to \"ready\" when done, then open a PR.",
      "What the gate does and does not prove: docs/authoring/shipping-gate.md",
