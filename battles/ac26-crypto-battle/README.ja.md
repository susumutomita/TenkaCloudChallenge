...
- **LEAK**: 計算なしで得点。ただし得点は低く、秘密の破片（share）が公開される。
- **PROVE**: 自分で計算して、より高い得点。share は公開されない。
- **どちらもしない**: 期限切れで減点。放置がいちばん損になります。

得点の順序はこうです。**何もしない < LEAK して狩られる < LEAK 無事 < PROVE**。
ORDER カードには「計算したときの点」と「パスしたときの点」が並べて出るので、
出す前に見比べられます。
