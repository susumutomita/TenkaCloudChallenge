...
`game/src/fhe.ts` の加法準同型暗号です。素体 F_p 上で、鍵 `k` に対して
...
`k' = (y - m') * r^-1` が存在します。したがって暗号文は平文について**情報を一切
持ちません** — 計算量的仮定ではなく情報理論的です。`fhe.test.ts` はこの主張を文章では
なく実行して確かめます。

**鍵は Order ごと**です。`deriveFheKey(seed, orderId, p)` が Order id に束縛するため、
ある Order について後から何を知っても別の Order は開きません（同じ鍵を使い回すと、
既知平文が 1 組出た時点で鍵が解けます）。
...
**なぜ嘘の答えを出せないか**: 判定は復号して期待値と比較します（#645 の判定要件）。
`(0, v)` を出せば `v` に復号されますが、平文は seed 由来の全域の体要素なので、期待する
和は約 2^61 分の 1 の当てずっぽうになります。それ以外の方法で正しい暗号文を作るには
鍵が要り、鍵は上記のとおり得られません。
