...
問題の解説は盤面より上に常時表示します。チュートリアルはその下の小さな「練習する（任意）」ボタンから開き、同じ場所で閉じられます。自動では開かず、練習せずにそのままプレーできます。数独 PROVE は付け替え表を選び、12 マスの見本を見ながら空欄 4 マスだけを埋めます。1〜4 の読み替えをそれぞれ 1 回ずつ行い、審判は完成した盤面全体を検査します。同じ表の使い回しが HUNT の手掛かりになるルールは変わりません。
