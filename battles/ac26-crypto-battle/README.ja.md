...
**Order を完了すると、その合計が実際に生成されます。これはレビュー由来の修正です。**
以前は生成されていませんでした — `applyMpc` は参加者の拠点の partial を記録して得点を
付けたら終わりで、statement が約束している「合計が求まる」という結果はテストの中にしか
存在しませんでした。参加者は計算をやり切ったのに、その意味を目にしないまま終わっていた
ということです。現在は artifact が他 2 拠点の partial と 3 つの和を持ち、ledger の行に 4 つの数が
**「割った余り」を明示した形で**並びます。足し算は参加者自身が確かめられます — それが
学ぶべき中身であって、結果だけを渡されることではありません。単に `a + b + c = 合計` と
書くと、3 つの和が `p` を超える場合（つまり大半の場合）に偽になり、しかも「手で確かめて
みたら合わない」という最悪の向きに偽になります。これらの公開は、partial 1 つの公開が
安全なのと同じ理屈で安全です。
