...
test("selection hint supplies all current operands without choosing a match",()=>{
 for(const text of Object.values(hint("encrypt"))) {
  ...
  expect(text).toContain("F(c6) = 1 =? 0");
  expect(text).not.toContain("≠");
 }
 expect(hint("encrypt").ja).not.toContain("候補番号 3");
});
test("decryption hint substitutes ciphertext factors while leaving the multiplication and matching to the player",()=>{
 for(const text of Object.values(hint("decrypt"))) {
  expect(text).toContain("(2, 5)");
  expect(text).toContain("2 × 2 = □");
  expect(text).toContain("S × 3 = □ → □");
  expect(text).toContain("S × 6 = □ → □");
  expect(text).not.toContain("4 × 3 = 12");
  expect(text).not.toContain("m = 3");
 }
});
test("probability hint lists only matching ticket counts but leaves the total to compute",()=>{
 for(const text of Object.values(hint("probability"))) {
  expect(text).toContain("3 + 1 + 2 = □");
  expect(text).not.toContain("3 + 1 + 2 = 6");
 }
});
