...
export const HINT_LADDER: Readonly<Record<OrderTaskKind, readonly HintSpec[]>> = {
  "anamorphic-rejection": [
    ...
    {id:"anamorphic-rejection/3",text:ctx=>{if(ctx.task.kind!=="anamorphic-rejection")throw new Error("wrong task");return {ja:`秘密の判定が${ctx.task.targetBit}になる最初の行番号を入力。その行のaに対応するsを下の表で読み、s×1〜6でbと同じ余りを探します。最後は受信パケットと同じ(a,b)の行の秘密ビットを入力します。`,en:`Enter the first row whose secret bit is ${ctx.task.targetBit}. Read s for its a from the power table; try s×1 through s×6 to match remainder b. Finally locate the incoming(a,b) and enter its lookup bit.`};}},
  ],
  "stark-trace": [
