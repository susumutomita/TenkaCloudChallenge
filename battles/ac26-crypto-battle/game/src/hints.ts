...
LEAK はかけらを公開記録に載せる操作。必要な個数を集めて秘密を戻す攻撃を HUNT と呼びます。${ctx.allowedMethods.includes("prove") ? "PROVE は別の秘密である数独の解を持つことを示し、このかけらを出さずに Order に答えます。" : "この Order は「公開が条件」です。依頼主がかけらそのものを買うので、答え方は LEAK だけ、得点は計算して答えたときと同じ満額です。代わりに、あなたの公開済みのかけらが 1 個増えます。"}`,
