...
function oneOrderPerKind(): { projection: CryptoBattleProjection; order: ContractProjection }[] {
  ...
  for(let t=0;t<=1_200_000&&(!seen.has("ec-add")||!seen.has("io-equivalence"));t+=30000){
    streaming=tick(streaming,t);
    const projection=projectForTeam(streaming,"teamA");
    for(const order of projection.myContracts)if(order.task.kind==="ec-add"||order.task.kind==="io-equivalence")seen.set(order.task.kind,{projection,order});
  }
  return [...seen.values()];
}
