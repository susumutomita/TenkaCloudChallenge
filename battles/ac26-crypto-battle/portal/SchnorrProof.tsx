...
export function SchnorrProof({order,teamId,locale,busy,onSubmit}:{order:ContractProjection;teamId:string;locale:"ja"|"en";busy:boolean;onSubmit:(op:CryptoBattleOp)=>void}) {
  ...
  return <section className="tc-input-panel" aria-label="Schnorr zero-knowledge proof">
    ...
    <p className="tc-schnorr-progress">{ja ? `このお題は2回の送信で1つの証明を作ります。最後の応答が正しければ完了・+${order.points}点です。暗号化した文を復号する操作ではありません。` : `Two submissions form one proof. A correct final response completes this Order for +${order.points} points. Neither step decrypts a message.`}</p>
