import { isEvolutionTaskShape } from "../game/src/evolution-model.ts";
...
export function isCryptoBattleProjection(value: unknown): value is CryptoBattleProjection {
  ...
  // see.
  if (typeof v.prime !== "string" || v.prime.length > 700 || !/^[1-9][0-9]*$/.test(v.prime) || BigInt(v.prime) < 2n) return false;
  ...
  if (v.myContracts.some(c => (c.task?.kind === "rsa-decrypt" || c.task?.kind === "enigma-encrypt" || c.task?.kind === "ecdsa-sign") && !isEvolutionTaskShape(c.task))) return false;
  ...
  if (v.myContracts.some(c => c.task?.kind === "io-equivalence" &&
    (!rsaValue(c.task.a,5) || c.task.a < 1 || !rsaValue(c.task.b,5) || !rsaValue(c.task.c,5) ||
      ...
      c.task.missing.length !== 2 || c.task.missing.some((n:unknown)=>!rsaValue(n,4)) || (c.task.rotations !== undefined && (!Array.isArray(c.task.rotations) || c.task.rotations.length !== 2 || c.task.rotations.some((n:unknown)=>!rsaValue(n,16) || Number(n)<1)))))) return false;
