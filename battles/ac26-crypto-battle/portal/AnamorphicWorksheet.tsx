...
export function AnamorphicWorksheet({task,locale,busy,wrongCost,onSubmit}:{task:AnamorphicTask;locale:'ja'|'en';busy:boolean;wrongCost:number;onSubmit:(answer:string)=>void}){
 ...
 <button disabled={busy||!values.slice(0,2).every(v=>/^[1-6]$/.test(v))||!/^[3-9]$/.test(values[2]!)} onClick={()=>onSubmit(values.join(' '))}>{ja?'選択・復号・確率を提出':'Submit selection, decryption and probability'}</button>
