...
export default function Lightning({ projection, order, locale, busy, onDeclare, onSelect }: {
  ...
  return <aside data-testid="lightning" style={{ border: "1px solid #a07c16", borderRadius: 8, padding: "8px 10px", margin: "8px 0", fontSize: 12 }}>
    ...
    <div>{ja ? "解答前に1題へ指定。計算回答（PROVE・CIPHER・FHE・MPC・EC・SNARK・iO・STARK）の正解だけ2倍。LEAK・期限切れ・ROTATEで使用終了。取り消し・重ね使用はできません。" : "Declare before answering one Order. Correct calculations (PROVE, CIPHER, FHE, MPC, EC, SNARK, iO, STARK) pay double. LEAK, expiry or ROTATE spends it; no undo or stacking."}</div>
