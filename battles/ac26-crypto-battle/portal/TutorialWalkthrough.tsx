...
export const PRACTICE_STEPS: readonly PracticeStep[] = [
  ...
  },
  {
    ...
    ja: {
      ...
      purpose: "Schnorr（シュノア）証明は、公開値 y に対応する秘密 x を知っていると示す手順です。ZK（ゼロ知識）は検証者にも秘密を教えない性質です。ここでは小さい数で、値を固定→質問→応答の順を練習します。候補が最大11通りなので、模型の合格は秘密を知っていた保証にはなりません。",
    ...
    en: {
      ...
      purpose: "A Schnorr proof demonstrates knowledge of secret x for public value y. Zero knowledge (ZK) means the verifier learns no secret. Practise the sequence: fix a value, receive a question, respond. With at most 11 possible secrets, acceptance in this model does not certify prior knowledge.",
  ...
  },
  {
    ...
    ja: {
      ...
      question: "4 を鍵 3 で暗号にした数字", result: "暗号にした数字は 1。これをシーザー暗号と呼びます。逆に 1−4=−3、6 を足すと鍵の 3 を求められます。元と暗号の組を公開すると、相手に鍵を復元される理由です。シーザー暗号のお題では『計算して答える』と『公開してすぐ答える（LEAK）』の得点・公開リスクを見て選びます。", retry: "7 から 6 を引いてください。",
    },
    en: {
  ...
  },
  {
    ...
    ja: {
      ...
      question: "先に封じる数字", result: "先に 2 を出します。相手を待たず、手 m=1 と隠す数 r=2 を審判へ渡すと、同じ計算で 2 になるか確かめてもらえます。審判は両者の開封をそろえて同時公開します。この順番を commit-reveal と呼びます。この小さな数では別の手への開け方も探せるので、後出しを防ぐには審判の同時公開が必要です。r は次回 0〜10 のくじから引き直します。", retry: "48−23=25。もう一度 23 を引いてください。",
    },
    en: {
