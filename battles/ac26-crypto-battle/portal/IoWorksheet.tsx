...
import {ioDistribution,ioTables} from '../game/src/io.ts';
export function IoWorksheet({task,locale,busy,onSubmit}:{task:IoTask;locale:'ja'|'en';busy:boolean;onSubmit:(answer:string)=>void}){
 const ja=locale==='ja';const [values,setValues]=useState(['','','','']);const tables=ioTables(task);
 ...
 return <section className="tc-input-panel" aria-label="iO definition worksheet">
  ...
  <p>{ja?'入力0→3の答えを並べた表を真理値表と呼びます。元の式を捨て、この表だけを使います。さらに0〜3から同じ確率でrを選び、表を右へr個回します。公開するのは「rと回した表」の組です。rは秘密鍵ではありません。':'A truth table lists the answers for every input from 0 through 3. Discard the source formula and keep this table. Choose r uniformly from 0 through 3 and rotate the table right by r places. Publish both r and the rotated table; r is not a secret key.'}</p>
  ...
  <table><thead><tr><th>r</th><th>{ja?'Aから公開する表':'Table published from A'}</th><th>{ja?'Bから公開する表':'Table published from B'}</th><th>{ja?'確率':'Probability'}</th></tr></thead><tbody>{[0,1,2,3].map(r=><tr key={r}><td>{r}</td><td>{displayed(tables[0],0,r)}</td><td>{displayed(tables[1],1,r)}</td><td>1/4</td></tr>)}</tbody></table>
  <p>{ja?'□には上で計算した各プログラムの答えを入れて考えます。rも含む公開データ全体を比べ、AとBで共通する組を数えてください。':'Use your calculated A and B outputs in the □ cells. Compare complete published data, including r, and count outcomes shared by A and B.'}</p>
