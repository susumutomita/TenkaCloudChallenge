...
const COPY = {
  ja: {
    ...
    moves: [
      ...
      {name:"ANAMORPHIC — 通常鍵と追加秘密",body:"秘密の表で目的のビットになる最初の暗号文候補を選び、通常鍵で読む数を計算します。最後は、目的のビットになる3行のくじの枚数を足して提出します。小さい数の選び直し模型であり、実用の安全性はありません。"},
  ...
  en: {
    ...
    moves: [
      ...
      {name:"ANAMORPHIC — Ordinary key and additional secret",body:"Select the first ciphertext trial matching the hidden bit, compute its ordinary decryption, then sum the ticket counts of the three rows matching the target bit. This tiny rejection-sampling model has no practical cryptographic security."},
