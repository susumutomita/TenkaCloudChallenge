  ...
  return (
    <section className="tc-move-shell" aria-label="crypto-battle-fast-moves">
      ...
      */}
      ...
      />}
      {selectedOrder?.task.kind === "stark-trace" && <StarkWorksheet key={`stark:${selectedOrder.id}`} task={selectedOrder.task} locale={locale} busy={submitting} onSubmit={answer=>void run(
      ...
      {selectedOrder?.task.kind === "io-equivalence" && <IoWorksheet key={`io:${selectedOrder.id}`} task={selectedOrder.task} locale={locale} busy={submitting} onSubmit={answer=>void run(
