...
export const FAST_MOVE_COPY = {
  en: {
    ...
    */
    ...
    huntCipherBody: "Recovered key accepted — this method is completed for the current generation.",
    rejected: "REJECTED",
    ...
    exposureSelf: "you",
    exposureSafe: "safe",
    exposureWarn: "at risk — public evidence is available",
    ...
    <section className="tc-move-shell" aria-label="crypto-battle-fast-moves">
      ...
      */}
      {(proveOpen || selectedOrder?.task.kind === "zk-sudoku") && selectedOrder?.schnorr && proveAllowed && <SchnorrProof key={`schnorr:${selectedOrder.id}`} order={selectedOrder} teamId={projection.vault.teamId} locale={locale} busy={submitting} onSubmit={op=>void runOrder(()=>client.submitOp(op),next=>{
        ...
        return proof?.kind === "proof" && proof.publicKey ? {kind:"prove",reward:next ? orderReward(selectedOrder,next) : selectedOrder.points,title:locale === "ja"?"模型の検証式が一致！":"Model equation verified!",body:`${locale === "ja" ? "検証式が一致" : "Verification matches"}: ${power(2,Number(proof.response))} = ${Number(proof.commitment)*power(Number(proof.publicKey),Number(proof.challenge))%23}。${locale === "ja"?"応答の計算に得点しました。この小さい模型では、秘密を知っていたことの保証にはなりません。":"Points awarded for the response calculation. This tiny model does not certify prior knowledge of the secret."}`} : {kind:"error",title:locale === "ja"?"検証式が一致しません":"Verification failed",body:selectedOrder.allowedMethods.length === 1 ? (locale === "ja" ? "このお題は不合格で終了しました。追加の期限切れ減点はありません。次のお題へ進んでください。" : "This Order ended with a failed proof. No additional deadline penalty applies. Continue to the next Order.") : (locale === "ja" ? "応答は1回だけです。期限までにLEAKへ切り替えるか、次のお題へ進んでください。" : "Only one proof response is accepted. Switch to LEAK before the deadline or continue to the next Order.")};
      })} />}
            ...
            {selectedOrder.hints.filter(hint => hint.text !== undefined).map(hint => (
              <details className="tc-hint-rung" key={`${hint.id}:${selectedOrder.hints.filter(h => h.text).length}`} open={hint.level === selectedOrder.hints.filter(h => h.text).length - 1}>
                ...
                {hint.level === 2 && !selectedOrder.schnorr && (selectedOrder.task.kind === "reveal-share" || selectedOrder.task.kind === "zk-sudoku") && selectedOrder.allowedMethods.includes("prove") ?
                  ...
                  <HintContent text={hint.text?.[locale] ?? ""} />}
              </details>
            ))}
            {nextHint ? (
              <button
