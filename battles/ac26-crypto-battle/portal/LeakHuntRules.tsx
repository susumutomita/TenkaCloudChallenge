...
export const LEAK_HUNT_RULES = {
  ja: {
    ...
    shareRule: "同じ相手・同じ世代（同じ組）の、異なる番号が必要です。同じ番号を何回公開しても1個。上の3・−3・1は番号1・2・3専用で、別の番号では画面の係数を使います。値×係数を全部足し、表示された割る数で割った余りを提出します。",
  ...
  en: {
    ...
    shareRule: "Use distinct indices from the same opponent and generation (set). Repeating an index counts once. Factors 3, −3, 1 apply only to indices 1, 2, 3; otherwise use the displayed factors. Add value×factor for each share and submit the remainder using the displayed divisor.",
