import FirstMission from "./FirstMission.tsx";
...
const TOPICS = ["first", "play", "concepts", "practice", "reference"] as const;
type Topic = typeof TOPICS[number];
const LABELS = {
  ja: { first: "まず1問体験", play: "遊び方", concepts: "暗号を図と式で見る", practice: "一桁の穴埋めで練習", reference: "ルール一覧" },
  en: { first: "Try your first mission", play: "How to play", concepts: "Cryptography in diagrams and formulas", practice: "One-digit practice", reference: "Rules reference" },
} as const;
...
export default function BattleHelp(props: PortalSlotProps) {
  ...
  const [conceptStart, setConceptStart] = useState<"fhe" | undefined>();
  const close = () => dialog.current?.close();
  const open = (next: Topic) => { setConceptStart(undefined); setTopic(next); dialog.current?.showModal(); };
  return <>
    <style>{`
      ...
      .tc-help-entry button.tc-help-first,.tc-help-dialog button.tc-help-control.tc-first-submit{background:#245986;color:#fff;border-color:#245986;min-height:44px;padding:10px 16px;font-weight:700}
      .tc-help-dialog button.tc-first-submit:disabled{background:#dce5ed;color:#344f67;border-color:#b8c9db;cursor:default}
    ...
    <nav className="tc-help-entry" aria-label={ja ? "任意の解説と練習" : "Optional explanations and practice"}>
      {TOPICS.map(item => <button type="button" key={item} className={item === "first" ? "tc-help-first" : undefined} aria-haspopup="dialog" onClick={() => open(item)}>{labels[item]}</button>)}
    </nav>
    <dialog ref={dialog} className="tc-help-dialog" aria-labelledby="tc-help-title" onClose={() => setTopic(null)}>
      ...
      <div className="tc-help-content">
        ...
        {topic === "first" && <FirstMission locale={locale} onLearn={() => { setConceptStart("fhe"); setTopic("concepts"); }} onDone={close} />}
        {topic === "play" && <QuickRules locale={locale} />}
        {topic === "concepts" && <ConceptExplanation locale={locale} topic={conceptStart} initialTopic={conceptStart} embedded />}
