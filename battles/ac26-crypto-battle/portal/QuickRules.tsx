...
export const QUICK_RULES_COPY: Record<Locale, QuickRulesCopy> = {
  en: {
    ...
    choice: "LEAK and PROVE are alternatives for the same request; some requests accept neither and ask you to compute on numbers you cannot read (FHE, MPC). HUNT and ROTATE are the attack and defence that follow.",
    steps: [
      { title: "1. Request arrives", body: "Check its points, requested index, and countdown." },
      { title: "2. Do what it asks", body: "LEAK now, PROVE the same request safely, or run the computation an FHE / MPC request needs." },
  ...
  ja: {
    ...
    choice: "LEAK と PROVE は同じ依頼に対する二択です。どちらも受け付けず、読めない数字のまま計算せよという依頼もあります（FHE / MPC）。HUNT と ROTATE は、その後の攻撃と防御です。",
    steps: [
      { title: "1. 依頼が届く", body: "得点、要求 index、残り時間を確認します。" },
      { title: "2. 依頼に応じる", body: "LEAK するか、同じ依頼を PROVE するか、FHE / MPC の依頼なら求められた計算をします。" },
