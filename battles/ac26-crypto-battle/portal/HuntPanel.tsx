...
export function huntOptions(projection: CryptoBattleProjection): readonly HuntOption[] {
  ...
  return Object.values(projection.teams).filter(team => team.teamId !== projection.vault.teamId).flatMap(team => {
    ...
    const rpsOption: HuntOption = { ...entry("rps"), rps, left, points: projection.rpsHunt?.winPoints, status: rps ? status("rps", true, left) : pending ? "pending" : left === 0 ? "exhausted" : lastRpsResult ? "completed" : "waiting", detail: {
      ja: rps ? "別の2対戦で同じ隠す数 r が公開され、今回の封じた数もあります。今回も同じ r という仮定で予測します。" : pending ? "予測は変更できません。両者の開封後に採点します。" : lastRpsResult ? "直近の予測は終了しました。次の対戦で、相手が数字を封じてから手を審判へ渡すまでの受付を待ちます。" : reusedR ? "過去の使い回しを確認。相手が次の数字を封じ、手を審判へ渡す前の受付を待ちます。" : `過去の開封 ${openings.length} 件。別の2対戦で同じ隠す数 r が公開されることが必要です。過去の開封は世代をまたいで参照できます。`,
      en: rps ? "Two past duels reused r, and the current sealed number is available. Predict assuming reuse again." : pending ? "Prediction is final; scoring waits for both openings." : lastRpsResult ? "The latest prediction is finished. Wait for the next duel to be sealed, before its opening is given to the judge." : reusedR ? "Past reuse found. Wait for the next sealed number, before the opponent gives their opening to the judge." : `${openings.length} public openings. Need equal r in two different duels. Past openings may span generations.`,
    }};
