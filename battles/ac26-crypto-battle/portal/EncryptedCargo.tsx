...
export default function EncryptedCargo({ locale, inputs, prime, left, right, onLeft, onRight }: {
  ...
  return <section className="tc-cargo" aria-label={ja ? "開けずに合計を届けよう" : "Deliver a sum without opening it"}>
    ...
    <p>{ja ? "暗号文は、中身の数を隠した「左・右の数の組」です。あなたは配送係。この組を、中身を開けずに足し合わせます。" : "A ciphertext is a pair of left and right numbers that hides a number inside. You are the courier: combine these pairs without opening their contents."}</p>
