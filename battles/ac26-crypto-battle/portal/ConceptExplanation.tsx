...
type Locale = "ja" | "en";
export type Concept = "enigma" | "rsa-decrypt" | "ecdsa" | "anamorphic" | "stark" | "io" | "snark" | "ec" | "schnorr" | "remainder" | "sharing" | "mpc" | "zk" | "fhe" | "caesar" | "vigenere" | "rsa" | "rotor" | "commit";
interface Step { readonly diagram?: "fhe" | "zk" | "relabel" | "sharing" | "mpc"; readonly title: string; readonly lines: readonly string[]; readonly table?: { readonly headers: readonly string[]; readonly rows: readonly (readonly string[])[] } }
...
export const EXPLANATIONS: Record<Locale, Record<Concept, Explanation>> = {
  ja: {
    "enigma": {"name": "エニグマの往復配線", "steps": [{"title": "車輪を進めて、表を往復する", "lines": ["車輪は数字を別の数字へ置き換える表です。1文字ごとに位置を進めるため、同じ数字でも変換が変わります。反射板は0と1、2と3を交換する表です。", "往路で車輪の表を左→右、反射板で交換、復路で同じ車輪を右→左に読みます。"]}, {"title": "具体例から逆の操作へ", "lines": ["位置0の表を0→1、1→3、2→0、3→2とします。元の数0は、往路1→反射0→復路2で、答え2です。", "同じ位置に戻し、2を入れると往路0→反射1→復路0。元に戻りました。"]}, {"title": "式で往復を表す", "lines": ["Wは車輪の表、Rは反射板、W⁻¹は表の逆引き。元の数mから暗号の数cを作る式はc=W⁻¹(R(W(m)))です。", "WとW⁻¹は打ち消し合い、Rを2回使うと元に戻ります。だから暗号化を同じ位置で2回行うと元に戻ります。実機の複数ローターや26文字を、1ローター・4数字に縮めた模型です。"]}]},
    "rsa-decrypt": {"name": "RSAの復号", "steps": [{"title": "秘密鍵を使って元の数を読む", "lines": ["暗号文cは中身を隠すために変換した数。平文mは元の数です。秘密鍵(n,d)を受け取り、m=c^dをnで割った余りで求めます。", "このお題の鍵は(n=15,d=3)。cを3回掛ける計算です。お題専用の鍵なのでHUNT対象外です。"]}, {"title": "小さな復号を手で計算する", "lines": ["c=8なら8×8=64。15で割った余り4にしてから、4×8=32。余り2が平文です。", "途中で15の倍数を引いても、その後の積が15の倍数だけ変わるので最後の余りは同じです。"]}, {"title": "公開鍵と秘密鍵の対応", "lines": ["公開鍵(n=15,e=3)ではm^3の15で割った余りを作ります。秘密鍵でも3乗するので、全体はm^9の15で割った余りです。", "15=3×5。3でも5でも割り切れない数は8乗の余りが1です。3や5の倍数も、それぞれの余りで確認すると元に戻ります。実用の安全な符号化を省いた教科書RSAです。"]}]},
    "ecdsa": {"name": "ECDSAの署名", "steps": [{"title": "秘密にするのではなく、対応を検証する", "lines": ["署名は、メッセージと鍵の対応を検査するための数字の組です。ECDSAは楕円曲線という点の足し算を使います。", "ハッシュはメッセージを短い数にする処理。このお題は処理済みの値h、秘密鍵d、今回だけの数kを配ります。"]}, {"title": "表から点と逆元を読む", "lines": ["Gは出発点。kGはGをk回足した点で、表に計算済みの点を示します。点(x,y)の左の数がx座標です。", "kの逆元k⁻¹は、kと掛けると7で割った余りが1になる数。2の逆元は4です。2×4=8の余りは1になります。"]}, {"title": "署名の2個を計算する", "lines": ["rはkGのx座標。s=k⁻¹×(h+d×r)を7で割った余りです。", "例：h=1,d=2,k=2。2G=(1,3)からr=1。s=4×(1+2×1)=12の余り5。署名は(1,5)。"]}, {"title": "なぜ秘密鍵を渡さず検証できるか", "lines": ["公開鍵Q=dG。検証者はsの逆元wを求め、X=(h×w)G+(r×w)Qを計算します。", "Qを代入するとX=s⁻¹(h+rd)G=kGです。したがってXのx座標をrと比較できます。無限遠点（点の足し算の0役）は不合格です。この小さい曲線と公開済みkは手計算用で、実用の安全性はありません。"]}]},
    ...
    commit: { name: "手を先に封じる", steps: [
      ...
      { title: "自分が封じたら、審判へ預ける", lines: ["自分の c を送ったら、相手を待たずに控えた m と r を審判へ渡します。審判が同じ計算をして c と一致するか確かめます。不一致なら修正でき、減点はありません。開封した手と r が受理された後は、その開封内容を変更できません。", "片方だけの開封は相手へ渡さず、両方そろってから同時に公開します。グーはチョキ、チョキはパー、パーはグーに勝ちます。"] },
    ...
    sharing: {"name": "秘密分散とシェア", "steps": [{"title": "シェアは、秘密を分けて持つためのデータ", "diagram": "sharing", "lines": ["秘密分散は、秘密を分けて保管する方法です。配る番号と数の組をシェア（share）と呼びます。", "秘密計算（MPC）は、互いの入力を明かさず協力して計算する技術です。秘密分散を使って作る方式もあります。"]}, {"title": "まず、足し算で分ける例", "lines": ["秘密 6 → シェア 1・2・3", "戻すとき：1 + 2 + 3 = 6。計算は7で割った余りを使います。", "1と2だけでは、残りが分からず秘密を決められません。この例は3個すべて必要です。"]}, {"title": "この試合では、5個のうち3個で戻す", "lines": ["小さい例（割る数7）：秘密1を、式 1 + 番号 × 番号 で分けます。", "番号1 → 2、番号2 → 5、番号3 → 10 → 7を引いて3。", "番号1・2・3から戻す式：3×2 − 3×5 + 3 = −6 → 7を足して1。別の番号を使うと、掛ける数も変わります。"]}, {"title": "公開した情報が、攻撃の材料になる", "lines": ["LEAK：シェアを公開して得点。相手にもその数が見えます。", "HUNT：相手の同じ世代（同じ秘密から作った一組）のシェアを集め、元の秘密を計算して攻撃します。", "このゲームでは、秘密分散と、覆面で合計を出す秘密計算を別のお題で体験します。"]}]},
    mpc: {"name": "秘密計算（MPC）", "steps": [{"title": "入力を隠したまま、一緒に計算する", "diagram": "mpc", "lines": ["MPCは、複数の人が自分の入力を互いに明かさず、協力して計算する技術です。", "例：各社の売上を見せず、合計だけ知る。合計から分かる情報は隠しません。"]}, {"title": "隠す数を足し引きする模型", "lines": ["マスク（覆面）は、2人の間だけで共有する乱数、つまりランダムに選ぶ数です。片方が足し、もう片方が同じ数を引きます。", "A：2 + 1 − 4 = −1", "B：3 + 4 − 2 = 5", "C：1 + 2 − 1 = 2"]}, {"title": "全員分を足すと、覆面が消える", "lines": ["(2 + 1 − 4) + (3 + 4 − 2) + (1 + 2 − 1)", "+1 と −1、+4 と −4、+2 と −2 が消える → 2 + 3 + 1 = 6。", "7で割った余りでも、−1→6、6+5+2=13→6。同じ結果です。"]}, {"title": "自分の小計を送る", "lines": ["小計＝自分の入力 + 受け取った覆面 − 送った覆面。", "最後にカードの p（割る数）で割った余りを、回答欄に1個入力します。", "この画面では自分の分だけ計算します。全員の合計は審判が確認します。"]}]},
    zk: {"name": "数独の模型（旧方式）", "steps": [{"title": "秘密を送らず、正しさを証明する技術", "diagram": "zk", "lines": ["ZKは、秘密情報を明かさずに、主張が正しいと確認してもらう証明です。例えば、パスワードそのものを送らず「知っている」と証明します。", "次の数独は「見せる情報を減らす」部分を学ぶ模型です。審判は解を知るため、この数独の手順は本物のZKプロトコルではありません。"]}, {"title": "数独の模型：数字だけを付け替える", "diagram": "relabel", "lines": ["同じ数字は、どのマスでも同じ数字へ。上の矢印を押して確かめます。", "各行・各列・太枠の箱で、1〜4が1回ずつ現れるルールは、そのまま残ります。"]}, {"title": "見せる範囲を小さくする", "lines": ["自分 → 付け替えた16マス → 審判が確認", "相手に見えるもの → 付け替え後の1行・1列・1箱のうち1組。元の解全体は公開しません。", "本物のZKでは、秘密を知る審判に頼らず証明を検証します。模型と技術の違いはここです。"]}, {"title": "この画面でやること", "lines": ["① 今回の置き換えを矢印で確認します。未使用の表が自動で用意されます。", "② 左と同じ位置の数字を、矢印の先へ読み替えて4マス入力。", "③「答えを送る」を押す。同じ表を再利用すると解が漏れる危険があるため、次回も新しい表を自動で用意します。"]}]},
    fhe: { name: "FHE", steps: [
      { title: "中身を読まずに、計算したい", diagram: "fhe", lines: ["準同型暗号は、隠した数に計算をして、あとで開くと中身に計算した結果が得られる仕組みです。FHE（完全準同型暗号）は足し算と掛け算を組み合わせられます。この問題では入口の『隠したまま足す』を体験します。", "隠した数を『暗号文』、隠すための秘密の数を『鍵』と呼びます。答えを検査する『判定側』だけが鍵を持ち、暗号文ごとに別々の鍵を使います。"] },
  ...
  en: {
    "enigma": {"name": "Enigma return wiring", "steps": [{"title": "Step, reflect, and return", "lines": ["A wheel is a substitution table. Stepping its position for each character changes the substitution. A reflector swaps0↔1 and2↔3.", "Read the wheel left→right, reflect, then read the same wheel right→left."]}, {"title": "Example and inverse", "lines": ["At position0, the table maps0→1,1→3,2→0,3→2. Input0 travels to1, reflects to0, and returns to2.", "Reset the position. Input2 travels to0, reflects to1, and returns to0."]}, {"title": "Equation", "lines": ["W is the wheel, R the reflector, and W⁻¹ the reverse lookup. c=W⁻¹(R(W(m))) maps original m to ciphertext c.", "W and W⁻¹ cancel; applying R twice cancels too. Repeating the operation at the same position restores the input. This four-digit, one-wheel model omits the full machine."]}]},
    "rsa-decrypt": {"name": "RSA decryption", "steps": [{"title": "Recover the original with a private key", "lines": ["Ciphertext c is transformed data; plaintext m is the original. Given private key(n,d), compute m=c^d, taking the remainder after division by n.", "Here(n,d)=(15,3). Multiply c three times. This task-only key is outside HUNT."]}, {"title": "Compute a small example", "lines": ["For c=8:8×8=64 leaves4 modulo15. Then4×8=32 leaves2. Plaintext2.", "Removing multiples of15 between multiplications leaves the final remainder unchanged."]}, {"title": "Why the keys correspond", "lines": ["Public key(15,3) cubes the plaintext modulo15. Private exponent3 cubes again, yielding m^9 modulo15.", "For numbers coprime to15, the eighth power leaves1. Multiples of3 or5 also round-trip when checked modulo each prime. This is textbook RSA without secure encoding."]}]},
    "ecdsa": {"name": "ECDSA signing", "steps": [{"title": "Create a verifiable pair", "lines": ["A signature is a pair used to check the correspondence between a message and a key. ECDSA uses point addition on an elliptic curve.", "A hash converts a message to a short value. The task supplies that value h, private key d, and one-use value k."]}, {"title": "Read points and inverses", "lines": ["G is the starting point; kG adds G k times. The table supplies those points. In(x,y), the left number is the x coordinate.", "The inverse k⁻¹ multiplied by k leaves remainder1 modulo7. For2 it is4, since2×4=8 leaves1."]}, {"title": "Calculate the pair", "lines": ["r is the x coordinate of kG. s=k⁻¹×(h+d×r), taking the remainder modulo7.", "Example h=1,d=2,k=2:2G=(1,3), so r=1. s=4×(1+2×1)=12 leaves5. Signature(1,5)."]}, {"title": "Verify without receiving d", "lines": ["Public key Q=dG. Compute the inverse w of s and point X=(h×w)G+(r×w)Q.", "Substitution yields X=s⁻¹(h+rd)G=kG. Compare its x coordinate to r; reject the point at infinity, the additive identity. The tiny curve and supplied nonce are for hand calculation, not security."]}]},
    ...
    commit: { name: "Commit-reveal", steps: [
      ...
      { title: "Seal, then privately give the judge your opening", lines: ["After your own commitment, give the judge your m and r without waiting for the opponent. The judge recomputes c. A mismatch can be corrected without a penalty; an accepted opening cannot be replaced.", "The judge keeps each opening private until both arrive, then publishes them together and scores the hands. Rock beats scissors, scissors beats paper, paper beats rock."] },
    ...
    sharing: { name: "Secret shares", steps: [
      { title: "One piece does not determine the secret", lines: ["Secret sharing splits a secret into numbered values called shares. A specified number can reconstruct it. MPC means computing together with private inputs; secret sharing can be a building block for MPC.", "Start with an addition-only example, taking remainders after dividing by 7: shares 1, 2, 3 give secret 1 + 2 + 3 = 6.", "Seeing only 1 and 2 leaves the last share anywhere from 0 to 6. Possible secrets are 3, 4, 5, 6, 0, 1, 2: all seven remain."] },
      ...
      { title: "What publication changes", lines: ["LEAK writes a requested share to the PUBLIC LEDGER, the record everyone can read. Enough shares from one generation (a set made from the same secret) allow recovery and HUNT. HUNT shows the published indices and missing evidence.", "Pieces from different generations cannot be mixed. The shortcut above is only for indices 1, 2, 3 with a three-share threshold, not other indices.", "LEAK itself needs no recovery calculation. Read ZK for the PROVE option."] },
    ] },
    mpc: { name: "MPC", steps: [
      { title: "Reveal the total, keep the inputs private", diagram: "mpc", lines: ["MPC means multiple parties computing together while keeping their own inputs private. Here three offices A, B and C want the remainder of their total after dividing by p. If p = 7 and the total is 9, the result is 9 − 7 = 2.", "Two offices privately share a random number called a mask. One adds it, the other subtracts it, so it disappears from the total.", "The adding side calls it received; the subtracting side calls it sent. Each office submits a subtotal."] },
...
export function conceptForTask(task: OrderTaskProjection): Concept {
  switch (task.kind) {
    ...
    case "rsa-decrypt": return "rsa-decrypt";
    case "enigma-encrypt": return "enigma";
    case "ecdsa-sign": return "ecdsa";
    ...
    <section aria-label="crypto-concept-explanation" style={{ color: "#16212e", background: "#fff", border: embedded ? undefined : "1px solid #d6e0eb", borderRadius: 6, padding: embedded ? 0 : 8, margin: embedded ? 0 : "8px 0", fontSize: 13 }}>
      ...
      {lesson && step && <div style={{ border: "1px solid #bad1e8", borderRadius: 6, background: "#f5f9fe", padding: 12, marginTop: 6 }}>
        ...
        <div aria-live="polite">
          ...
          {step.lines.map((line) => <p key={line} style={{ margin: "8px 0", lineHeight: 1.7 }}><MathText>{line}</MathText></p>)}
          ...
          {stepIndex === lesson.steps.length - 1 && task && prime && conceptForTask(task) === selected &&
            <div style={{ borderLeft: "3px solid #5597cd", paddingLeft: 10 }}>
              {orderCalculation(task, prime, locale).map((line) => <p key={line} style={{ margin: "6px 0" }}><MathText>{line}</MathText></p>)}
            </div>}
        </div>
