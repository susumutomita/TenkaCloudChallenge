...
export function StarkWorksheet({task,locale,busy,wrongCost,onSubmit}:{task:StarkTask;locale:'ja'|'en';busy:boolean;wrongCost:number;onSubmit:(answer:string)=>void}){
 ...
 <button type="button" className="tc-submit-small" disabled={busy||!values.every(v=>/^[0-6]$/.test(v))} onClick={()=>onSubmit(values.join(' '))}>{ja?'4個の計算を提出':'Submit four calculations'}</button>
