...
既存の `uptime-flat` で、正常な採点1回につき+100点、失敗時−100点。通常の1分周期なら1分ごとの得点です。登録前の既定値は空で、デプロイ成功だけでは得点しません。説明による追加得点はありません。別枠のチーム間交換は、下記のとおり最大+60点です。正式なスコア・順位・終了時刻はポータルで確認します。4回の復旧と振り返りに約30〜45分を見込み、準備約40分を別に確保します。実測前の目安です。

## 4人のカードとチーム間交流

運営がA〜DのカードURLを1人に1枚ずつ渡します。自分の手がかりだけが見え、合計・通信経路の並べ替え・2人組どうしの鍵の交換を相談します。4枚すべての確認がそろってから、担当カードで復旧を確認し、別の担当カードで説明します。4ラウンドで操作担当がA→B→C→Dと交代します。AIが計算できても、他のカードの確認はそのカードの持ち主に依頼します。

ポータルの **隣のチームと、つながろう** では、別チームと合言葉を交換します。双方が相手の合言葉を送ると、それぞれ+20点。同じ相手との重複加点はなく、異なる3組まで、各チーム最大+60点です。上限に達したチームも、初めての相手を手伝えます。待機・取消・拒否による減点はありません。既存のcoordination機能で、稼働点を残したまま交流点を加算します。

カードの所持を確認する仕組みであり、異なる人間であることを証明するものではありません。1人に全カードや全チームの認証情報を渡せば代行できます。運営は別々に配布し、相談を促してください。欠席者が出た場合は、運営用操作で確認を補助したことを画面に残し、不在の担当カードを在席者に再配布できます。手順はランブックにあります。
...
Python 3.12 以降、Node.js、Bun 1.3.11 を用意します。
