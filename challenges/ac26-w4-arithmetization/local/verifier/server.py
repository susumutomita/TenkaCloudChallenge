...
def _run_submission(submission: object, phases: tuple[str, ...], seed: str) -> tuple[bool, str | None]:
    source = submission.get('air.py') if isinstance(submission, dict) else submission
    if not isinstance(source, str) or not source.strip() or len(source) > MAX_BODY_BYTES:
        return False, None
    try:
        with LearnerSession({'air.py': source}, timeout=RUN_TIMEOUT_SECONDS) as learner:
            module = learner.module()
            failures = []
            if phases:
                for name in phases:
                    failures.extend(getattr(check_air, name)(module, seed))
            else:
                failures = check_air.run(module, seed)
