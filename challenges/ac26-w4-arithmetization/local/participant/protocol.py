...
def encode(value, depth=0):
    ...
    if isinstance(value, (set, frozenset)):
        return ['frozenset' if isinstance(value, frozenset) else 'set', [encode(item, depth+1) for item in value]]
...
def decode(frame, depth=0):
    ...
    if kind == 'str' and type(value) is str:
        return value
    if kind in ('bytes', 'bytearray') and type(value) is str:
        data = bytes.fromhex(value)
        return bytearray(data) if kind == 'bytearray' else data
    if kind in ('set', 'frozenset') and type(value) is list:
