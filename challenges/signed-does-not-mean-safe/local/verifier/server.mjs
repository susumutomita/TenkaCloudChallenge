...
const server = createServer({ maxHeaderSize: HEADER_LIMIT }, async (request, response) => {
  ...
  if (request.method === "POST" && ["/public-test", "/prepare"].includes(url.pathname)) {
    ...
    const report = runPublicCases(body.triage);
    const payload = { report };
    if (url.pathname === "/prepare" && report.correct) payload.submission = encodeSubmission(body.triage);
  ...
  const triage = decodeSubmission(body.submission);
  const report = triage
    ? gradeCheckpoint(triage, body.checkpointId)
    : { checkpointId: body.checkpointId, correct: false, errors: ["invalid submission"] };
  return send(response, 200, {
    checkpointId: typeof body.checkpointId === "string" ? body.checkpointId : "",
    correct: report.correct === true,
    message: report.correct
      ? "Triage boundary accepted."
      : "The checkpoint still has a counterexample.",
  });
});
