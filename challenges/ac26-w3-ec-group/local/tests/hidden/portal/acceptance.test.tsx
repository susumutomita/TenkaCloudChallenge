...
const source = readFileSync(root + '/local/tests/hidden/reader-curve.py', 'utf8');
const files = { 'curve.py': source };
const base = process.env.AC26_WORKBENCH_URL ?? 'http://127.0.0.1:18172';
...
it('edits the real starter, runs public examples, submits all eight checkpoints and collapses solved inputs', async () => {
  expect(createHash('sha256').update(source).digest('hex')).toBe('9dae080a9bf9639ab094f6e32a5fcc7746acf740c5fd61c23bac6a0cea716592');
  ...
  const editor = await screen.findByLabelText('curve.py');
  ...
  expect(mocks.prepareWorkbench).toHaveBeenCalledTimes(8);
