...
here is graded; it ships in the participant image on purpose, and the learner is meant to
read it.

Issue 537/538 (Issue 543 option B2): until that split this file's contents lived in
`fixtures/generate.py`, in the single Docker stage a learner's own `make build` produced --
so `setting`, `coefficients`, `witness`, `relation` and `value_catalog`, the five derivations
the hidden labels `h0`..`h3` are drawn from, shipped beside `tests/hidden/check_prover.py`
and `fixtures/specimens.py`. Those three files are the answer: the hidden checker's
`_expected_class`, `_authorized` and `_expected_leakage` are the rules `classify`, `open-set`
and `leakage` are graded against, and `fixtures/specimens.py`'s `GROUND_TRUTH` is a per-
specimen table of the capabilities reached, the unauthorized openings, the disclosed
`(channel, name)` pairs and the recoverable secret -- one row for each of the eight provers
the learner is asked to audit. They live behind the verifier now (see ../Dockerfile); what
travels to the participant is this file, the specimens themselves, the bench, and the values
in `GET /public`.
...
coefficients, no fixtures, no skeleton.
"""
...
# ---------------------------------------------------------------------------
# The two supplied answers, and the round they are allowed to talk in
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# The vocabulary a data-classification policy is written against
# ---------------------------------------------------------------------------
