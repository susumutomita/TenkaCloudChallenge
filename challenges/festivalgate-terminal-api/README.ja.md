...
5. 問題instructionsで案内されるprofileの入口に従い、端末tokenでcustomer recordをid指定
   (例: `/api/terminal/customers/1`) で開きます。隣接する`/api/terminal/customers/<id>`の
   小さな整数IDは列挙可能なので、各recordのname/roleを見比べて特異なrecordを見つけます。
