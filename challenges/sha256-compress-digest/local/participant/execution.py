...
class LearnerSession:
    ...
    def module(self):
        return SimpleNamespace(**{name: (lambda *args, _name=name: self.call('compress', _name, args))
                                  for name in ('round_step', 'compress_rounds', 'compress_block', 'invert_round', 'invert_rounds', 'sha256_hex')})
