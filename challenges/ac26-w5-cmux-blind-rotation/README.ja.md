...
8項目すべてに各2点の3段ヒントを追加します（計48点）。列の計算・回転・記録の規則は無料本文で説明します。出力の不一致は観察事項であり、秘密を使う分岐がない証明ではありません。独立した参加者資料の読解で、この過大な主張・API不足・maskが0の記録の例外を発見しました。採点器の実行処理は変更せず、ヒントの範囲と減点を変更します。
...
作者検証：`make reference-test` で参考解が通り、既存の計算誤り 24 件をすべて拒否しました。`make boundary-test` は 10 個の Linux テストで、全8項目の参考解、未実装・途中終了、型の保持、期限、ファイル/プロセス分離、非 root 実行を検査します。実 Workbench の HTTP 経路（config → inspect → starter → test → prepare → verify）で参考解の公開例 4 件と全8項目の提出が通り、途中終了は全項目で拒否されました。作者による回帰検証であり、独立した参加者プレーや実 Portal の得点履歴の検証ではありません。
