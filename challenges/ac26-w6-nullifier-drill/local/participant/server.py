...
PROBLEM_ID = "ac26-w6-nullifier-drill"
...
PORT = int(os.environ.get("WORKBENCH_PORT", "18167"))
...
CHECKPOINTS = ('label', 'repeat', 'scopes', 'accept', 'count', 'message', 'unchecked', 'collision')
...
_WORKBENCH = PortalEditorSupport(
    ...
    problem_id='ac26-w6-nullifier-drill',
    problem_name='名前を出さずに、二票目を見分ける',
    problem_name_en='Detect a second vote without a name',
    description='一人一票の受付を点検。小さな数で確認し、後半は正しい票を消す受付順と、四方式を区別する五票を自分で作ります。',
    description_en='Audit a one-vote-per-person service with small numbers, then construct an order that blocks valid votes and five votes that distinguish four designs.',
    checkpoint_labels={'label': '使用済みを見分ける印', 'repeat': '投票内容を変えても同じ印', 'scopes': '今回の二つの投票番号を比べる', 'accept': '確認してから使用済みにする', 'count': '受理後の記録を数える', 'message': '投票内容を印に混ぜるとどうなるか', 'unchecked': '正しい二票を消す受付順を作る', 'collision': '四方式を見分ける五票を作る'},
    checkpoint_labels_en={'label': 'Calculate a spent marker', 'repeat': 'Keep the marker when the vote changes', 'scopes': 'Separate elections', 'accept': 'Check before marking as spent', 'count': 'Count the resulting records', 'message': 'Test a marker that includes the vote', 'unchecked': 'Construct an order that erases two valid votes', 'collision': 'Construct five votes distinguishing four designs'},
    submitted_files=('nullifier_drill.py',),
    code_checkpoints=(),
