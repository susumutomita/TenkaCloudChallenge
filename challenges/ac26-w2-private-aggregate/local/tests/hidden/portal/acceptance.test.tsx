...
const files=Object.fromEntries(['aggregate.py'].map(name=>[name,readFileSync(reader+'reader-'+name,'utf8')]));
const base=process.env.AC26_WORKBENCH_URL ?? 'http://127.0.0.1:18153';
...
beforeEach(async()=>{
 ...
 mocks.submitFlag.mockImplementation(async(_a,_s,_p,submission,checkpointId)=>{
  ...
  return {kind:'ok',scoreDelta:metadata.scoring.checks.find((c:any)=>c.id===checkpointId).points,totalScore:300};
 });
});
...
it('submits all eight code fields through real Portal components and local API',async()=>{
 const user=userEvent.setup();show(flags);await screen.findByLabelText('aggregate.py');
 expect(screen.getAllByText('This checkpoint submits the current source from the editors above.')).toHaveLength(8);
