...
def main():
    ...
    source = (Path(__file__).parent / 'portal/reader-aggregate.py').read_text()
    assert hashlib.sha256(source.encode()).hexdigest() == '68f80983819ee810e543608dda56cfbc105488a58a1d4de2ce39b41aa6ea79a3'
    ...
    def verify(name, submitted, checkpoint='plan', expected=True):
        started = time.monotonic()
        prepared = post('/api/prepare', {'files': {'aggregate.py': submitted}})
    ...
    for checkpoint in ('plan', 'share-inputs', 'linear', 'multiply', 'result', 'privacy', 'cost', 'transfer'):
