...
Untrusted JSON function results and opening requests cross this boundary;
...
class LearnerSession:
    ...
    def call(self, module, function, args, modulus=None):
        ...
        opening = args[-1] if function == 'aggregate' else None
        wire_args = args[:-1] if opening is not None else args
        opening_id = 0
        opening_bytes = 0
        request = {'callId': self.sequence, 'module': module, 'function': function,
                   'args': wire_args, 'modulus': modulus, 'withOpening': opening is not None}
        try:
            self._send(request)
        ...
        while True:
            ...
            if isinstance(value, dict) and value.get('callId') == self.sequence:
                if 'opening' in value:
                    if opening is None or type(value.get('openingId')) is not int or value['openingId'] != opening_id:
                        raise LearnerError('The opening channel was malformed.')
                    opening_bytes += len(line.encode())
                    if opening_bytes > MAX_LOG_BYTES:
                        raise LearnerError('Opening output exceeded the limit.')
                    # This trusted object never enters the learner. Record each actual
                    # request before replying; child attributes/stdout are not counters.
                    try:
                        opened = opening.open_batch(value['opening'])
                    except (ValueError, TypeError):
                        raise LearnerError('The opening channel received invalid sharings.') from None
                    self._send({'callId': self.sequence, 'openingId': opening_id, 'opened': opened})
                    opening_id += 1
                    continue
                if 'error' in value:
    ...
    def module(self):
        def function(name):
            return lambda *args: self.call('aggregate', name, args)
        return SimpleNamespace(**{name: function(name) for name in
                                  ('plan', 'share_inputs', 'add_public', 'aggregate')})
