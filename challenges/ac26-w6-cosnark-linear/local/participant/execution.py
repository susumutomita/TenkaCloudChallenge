...
from participant.mpc import ParticipantRuntime, CrossPartyRead
...
class LearnerSession:
            ...
            while True:
                ...
                if isinstance(value, dict) and value.get('callId') == self.sequence:
                    if 'runtime' in value:
                        ...
                        if (not isinstance(runtime, ParticipantRuntime)
                        ...
                        allowed = ('value_of', 'add', 'mul_public', 'zero', 'events', 'violations', 'ancestry', 'issued')
                        ...
                        except (ValueError, TypeError, CrossPartyRead) as error:
    ...
    def module(self):
        return SimpleNamespace(**{name: (lambda *args, _name=name: self.call(_name, args)) for name in ('parse_relation', 'validate_shared_witness', 'shared_linear_combination', 'prove_linear', 'no_reconstruction_report', 'communication_report', 'sparse_counterexample')})
