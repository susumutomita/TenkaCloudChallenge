...
_WORKBENCH = PortalEditorSupport(
    ...
    seed=SEED,
    problem_id='ac26-w6-cosnark-linear',
    problem_name='共同証明の準備：秘密を分けたまま足す',
    problem_name_en='Prepare a joint proof: add without combining secrets',
    description='最初は prover.py の parse_relation を実装し、relation を提出します。秘密を分けたまま計算し、記録から出所と通信を確認する練習です。',
    description_en='First implement parse_relation in prover.py and submit relation. Practice computing on shares and checking their origin and communication from records.',
    checkpoint_labels={'relation': '公開の指示書を読む', 'witness': '値を読まずラベルを照合する', 'combine-a': '各自のシェアを掛けて足す', 'combine-b': 'A と B を別の係数で作る', 'audit': '結果の出所を監査する', 'trace': '記録から通信を数える', 'equivalence': '分け方を変えて結果を確かめる', 'transfer': '未見の設定と、自作の反例で確かめる'},
    checkpoint_labels_en={'relation': 'Read the public instruction row', 'witness': 'Match labels without reading values', 'combine-a': 'Scale and add each holder’s shares', 'combine-b': 'Build A and B with different coefficients', 'audit': 'Audit where results came from', 'trace': 'Count communication from the record', 'equivalence': 'Check results after sharing changes', 'transfer': 'Check unseen settings and build a counterexample'},
