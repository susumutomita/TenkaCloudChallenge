...
and its value is readable only through the runtime, which knows which party is currently
computing. Reading another party's share is not an error the learner has to remember to
avoid -- it is recorded, and the checkpoint fails.
...
absent from what a participant is given:
  ...
  * no `reconstruct`. The capability is not on the participant's runtime at all, so
    "reconstruct, do the arithmetic in the clear, re-share" is not an implementation that can
    be written rather than one that is asked for politely.
...
None of this is a security boundary -- the participant owns the machine and the image. It is
an *instrument*: it makes the difference between a local computation and a communicating one
visible in a log, which is the thing the problem is teaching.
"""
...
class Share:
    ...
    The value is deliberately not a plain attribute. Reading it goes through
...
def share_secret(seed: str, label: str, secret: int, setting: dict) -> tuple[Share, ...]:
    """Split `secret` into one `Share` per party, summing to it mod p."""
    s = _stream(seed, f"shares:{label}")
    prime, parties = setting["p"], setting["parties"]
    head = [_pick(s, 2 * i, 0, prime - 1) for i in range(parties - 1)]
    values = [*head, (secret - sum(head)) % prime]
    return tuple(
        Share(party, setting["fieldId"], f"{label}#{party}", value)
        for party, value in enumerate(values)
    )


def shared_witness(seed: str, setting: dict, witness, label: str = "w") -> tuple[tuple[Share, ...], ...]:
    """One sharing per witness element, indexed `[j][party]`."""
    return tuple(
        share_secret(seed, f"{label}{index}", value, setting)
        for index, value in enumerate(witness)
    )


def reconstruct(shares, prime: int) -> int:
    """Sum the shares. Present in the fixtures and **never** on a participant runtime."""
    return sum(share._value for share in shares) % prime  # noqa: SLF001 - author side


# ---------------------------------------------------------------------------
# Supplied: the instrumented runtime
# ---------------------------------------------------------------------------
...
class Runtime:
    """The only way to touch a share's value, and the log of everything that happened.
    ...
    def value_of(self, share: Share) -> int:
        ...
        if share.field != self.setting["fieldId"]:
            ...
            raise CrossPartyRead(f"share is in {share.field}, the setting is in {self.setting['fieldId']}")
    ...
    def mul_public(self, share: Share, constant: int) -> Share:
        ...
        value = (self.value_of(share) * int(constant)) % self.setting["p"]
        return self._emit("mul-public", (share,), value, public=int(constant))
    ...
    def _emit(self, operation: str, operands: tuple, value: int, public: int | None = None) -> Share:
        self._counter += 1
    ...
    def rounds(self) -> int:
        """Communication rounds. Structurally zero: nothing here can set the flag."""
    ...
    def summary(self) -> dict:
        return {
            ...
            "parties": sorted({event["party"] for event in self.events}),
        }

    def ancestry(self, share: Share) -> set[str]:
        """Every operand id the result was built from, transitively.

        This is what makes "did not reconstruct" checkable rather than promised. A share
        returned by an honest implementation traces back, through runtime events, to the
        input shares of **one** party and nothing else. Reconstruct-and-reshare cannot
        produce that: either the result is a `Share` this runtime never issued, so it has no
        ancestry at all, or reaching every party's value required reads this runtime refused.
        """
        ...
        return seen - {share.id} if share.id in produced else set()
...
class ParticipantRuntime:
    ...
    A deliberately narrow façade rather than the `Runtime` itself. The capability to put a
    sharing back together is not withheld by convention or by a test that notices afterwards
    -- it is not an attribute of this object, so the shortcut cannot be written.
    """
...
def coefficients(seed: str, label: str, setting: dict, shape: str = "dense") -> tuple[int, ...]:
    ...
    if shape == "sparse":
        drawn = [_pick(s, 2 * j, 0, prime - 1) if _pick(s, 2 * j + 40, 0, 3) == 0 else 0
...
def linear_shared(runtime: Runtime, coefficient_vector, shares) -> tuple[Share, ...]:
    """Ground truth: `[sum_j c_j w_j]`, one share per party, all local.

    For each party, start at that party's zero and fold in `c_j * [w_j]`. Every operation is
    inside that party's scope, so no share of another party is ever read -- and the event log
    that comes out has `communication: False` on every row, because there is nothing in the
    runtime that could set it otherwise.
    """
    out = []
    for party in range(runtime.setting["parties"]):
        with runtime.party_scope(party):
            total = runtime.zero()
            for coefficient, sharing in zip(coefficient_vector, shares):
                total = runtime.add(total, runtime.mul_public(sharing[party], coefficient))
            out.append(total)
    return tuple(out)


def prove_linear(runtime: Runtime, relation: dict, shares) -> dict:
    """Both halves of the row, as sharings. The whole of what this problem asks for."""
    return {
        "A": linear_shared(runtime, relation["a"], shares),
        "B": linear_shared(runtime, relation["b"], shares),
    }
