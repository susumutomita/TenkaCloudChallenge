...
one of this problem's eight checkpoints is graded on: `_Scenario.canonical` is the parser's
answer and the `malformed` list beside it is the whole of what that stage must refuse,
...
terms, and `check_audit` gives the five values an honest run reports beside the three runtimes
named for what each one catches. `fixtures/generate.py` shipped there too, with the `setting`,
`coefficients`, `witness` and `relation` the hidden labels `h0`..`h3` are drawn from. A
submission transcribed from those two files, with no reasoning past copying, scored all eight
...
# drops the whole scoring object rather than truncating it. This problem's eight stages fit
# exactly, so unlike `ac26-w5-pbs-homnand` nothing here is graded as a pair. Splitting a stage
# later means merging another one, not raising the cap on one side.
CODE_CHECKPOINTS = {
    "relation": ("check_relation",),
    "witness": ("check_witness",),
    "combine-a": ("check_combine_a",),
    "combine-b": ("check_combine_b",),
    "audit": ("check_audit",),
    "trace": ("check_trace",),
...
# `participant/mpc.py`, and `reference/prover.py` imports `field_id` from it at module level.
# The guard below takes the problem root off `sys.path`, so without preloading it here that
# import would not resolve and every checkpoint would fail on an import error. It stays in
...
# re-exports the supplied layer, so an unguarded `from fixtures.generate import *` would
# also pull in `setting`, `witness` and `relation`, which are what the hidden labels are
# drawn from.
_hidden_modules = {{
...
def main() -> None:
    # Not published to the host at all: `docker-compose.yml` puts this service on the
    # internal `lab` network only, so the port below is reachable from the Workbench
    # container and from nowhere else. The published port a participant uses is the
    # Workbench's, and it has not changed.
    port = int(os.environ.get("VERIFY_PORT", "18157"))
