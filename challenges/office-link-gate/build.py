...
def template():
    ...
    for name in ["PlayKey", "ProgressKey"] + [n + "Secret" for n in NAMES]:
        parameters[name] = {"Type": "String", "NoEcho": True, "MinLength": 24, "MaxLength": 64, "AllowedPattern": "^[A-Za-z0-9]+$"}
    variables = {"PLAY_KEY": {"Ref": "PlayKey"}, "PROGRESS_KEY": {"Ref": "ProgressKey"}}
    ...
    return {"AWSTemplateFormatVersion": "2010-09-09", "Description": "Office Link Gate: complete all four knowledge checks before receiving one flag. No real document changes.", "Parameters": parameters, "Resources": resources, "Outputs": outputs}
