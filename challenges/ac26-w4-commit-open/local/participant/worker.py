...
# This problem has no supplied module.
...
def initialization_error(error, filename):
    ...
    if isinstance(error, SyntaxError) and error.filename == filename:
        ...
        while tb is not None:
            if tb.tb_frame.f_code.co_filename in ('commit.py',):
...
def main():
    ...
    restrict_learner()
    modules = {name: types.ModuleType(name) for name in ('commit',)}
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            if name.startswith('@new:'):
                if len(sessions) >= 128:
                    raise ValueError('Too many protocol sessions.')
                sessions[name[5:]] = module.Session(*args)
                value = None
            elif name.startswith('@method:'):
                _, token, method = name.split(':')
                if method not in ('commit', 'receive_challenge', 'open'):
                    raise ValueError('Unknown protocol operation.')
                value = getattr(sessions[token], method)(*args)
            else:
            ...
            response = {'callId': call['callId'], 'error': 'ProtocolError' if isinstance(error, getattr(modules['commit'], 'ProtocolError', ())) else type(error).__name__}
