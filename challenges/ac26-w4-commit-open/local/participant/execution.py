...
class LearnerSession:
    ...
    def module(self):
        def function(name):
            return lambda *args: self.call('commit', name, args)
        module = SimpleNamespace(**{name: function(name) for name in ('encode_leaf', 'leaf_hash', 'node_hash', 'merkle_root', 'open_at', 'verify_opening', 'transcript', 'challenge', 'adaptive_witness', 'ambiguity_witness', 'lenient_opening')})

        def session(values):
            token = secrets.token_hex(16)
            self.call('commit', '@new:'+token, (values,))
            return SimpleNamespace(**{method: (lambda *args, _method=method: self.call('commit', '@method:'+token+':'+_method, args))
                                     for method in ('commit', 'receive_challenge', 'open')})
