...
`tests/hidden/check_commit.py` shipped in the learner's own image alongside it, and
every checkpoint is graded by running that suite. `fixtures/generate.py` shipped there
too, and its `node_hash` is a complete Merkle node-combining function under the exact
name the starter's own `node_hash` stub asks the learner to write. That Portal-facing
surface now lives in `participant/server.py`, in a separate image (see ../Dockerfile)
that this process's own container never builds; this file, `fixtures/` and
...
def _run_submission(submission: object, phases: tuple[str, ...], seed: str) -> tuple[bool, str | None]:
    source = submission.get('commit.py') if isinstance(submission, dict) else submission
    if not isinstance(source, str) or not source.strip() or len(source) > MAX_BODY_BYTES:
        return False, None
    try:
        with LearnerSession({'commit.py': source}, timeout=RUN_TIMEOUT_SECONDS) as learner:
            module = learner.module()
            failures = []
            if phases:
                for name in phases:
                    failures.extend(getattr(check_commit, name)(module, seed))
            else:
                failures = check_commit.run(module, seed)
...
def _unwrap_submission(checkpoint_id: str, submission: object) -> object:
    ...
    as ac26-w5-lwe-rlwe's verifier, for the same reason.
