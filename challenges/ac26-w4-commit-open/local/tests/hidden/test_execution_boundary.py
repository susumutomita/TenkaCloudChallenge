...
class ExecutionBoundaryTests(unittest.TestCase):
    ...
    def test_legal_alternative(self):
        source = REFERENCE + "\ndef adaptive_witness(setting):\n    values = [value+2 for value in setting['values']]\n    values[setting['query']] = setting['values'][setting['query']]\n    return {'values':values, 'query':setting['query']}\n"
        self.assertEqual(evaluate_with_message('adaptive', source), (True, None))
