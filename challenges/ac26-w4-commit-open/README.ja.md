...
全8項目を各2点の3段ヒントに変更しました。参加者向け資料だけの読解で、ハッシュ衝突・位置の固定・公開順序の誤保証と、path長・Eのバイト変換の手順不足を確認しました。日英の無料本文で訂正・補足しています。カタログ検証は通過。説明を確認した時点では実 Portal プレーは未実施です。今回の実行境界の検証は下記に記録します。
...
変更後の Linux Docker で、論理変異 20 件を検出し、実行境界回帰 8 件も成功しました。
