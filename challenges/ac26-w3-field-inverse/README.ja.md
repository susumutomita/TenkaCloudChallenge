...
例外の観測と応答の生成は、隔離した worker 内の小さな CPython ネイティブアダプターで行います。提出コードの Python フレームやクロージャーから、この処理を差し替えることはできません。呼び出した Python 関数から実際に返された例外型を観測しますが、提出コードが独自の通信形式を書き出す場合にその由来を認証するものではありません。コンパイラーは Docker のビルド段階だけで使い、実行用イメージには入りません。このアダプターには有限体の解法や非公開テストを実装していません。
