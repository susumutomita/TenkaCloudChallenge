...
const source = readFileSync(root + '/local/tests/hidden/portal/reader-field.py', 'utf8');
const files = { 'field.py': source };
const base = process.env.AC26_WORKBENCH_URL ?? 'http://127.0.0.1:18100';
...
it('edits the real starter, runs public examples, submits all seven fields and collapses solved inputs', async () => {
  expect(createHash('sha256').update(source).digest('hex')).toBe('d95f8913442ea7879fc8447a91f2a5c1c9b2c4c35d81c9389ac91fffe3dac2ee');
  ...
  const editor = await screen.findByLabelText('field.py');
  ...
  await screen.findByText(/prime modulus/);
