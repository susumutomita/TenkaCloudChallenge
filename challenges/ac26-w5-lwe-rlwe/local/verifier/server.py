...
Security contract (docs/curricula/advanced-cryptography-2026/TEMPLATE.md §/verify):
  ...
  - The runner prints its verdict and then hard-exits, so nothing registered during the
    submission's import gets another turn at stdout. See `scripts/verifier-spoof-guard.test.ts`.
...
from tests.hidden import check_lattice
try:
    import lattice
...
if phases:
    failures = []
    for name in phases:
        failures.extend(getattr(check_lattice, name)(lattice, {seed!r}))
else:
    failures = check_lattice.run(lattice, {seed!r})
...
def _run_submission(submission: object, phases: tuple[str, ...], seed: str) -> bool:
    ...
    if isinstance(source, dict):
        source = source.get("lattice.py")
    if not isinstance(source, str) or not source.strip():
        return False
    if len(source) > MAX_BODY_BYTES:
        return False
    with tempfile.TemporaryDirectory() as workspace:
        (Path(workspace) / "lattice.py").write_text(source, encoding="utf-8")
...
def evaluate(checkpoint_id: str, submission: object) -> bool:
    if checkpoint_id in CODE_CHECKPOINTS:
        # `transfer` runs under a derived seed, so its ring, dimension, secrets and cases
        # are not the ones any other checkpoint used.
...
def main() -> None:
    port = int(os.environ.get("VERIFY_PORT", "18108"))
