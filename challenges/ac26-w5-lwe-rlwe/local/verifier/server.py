...
def _run_submission(submission: object, phases: tuple[str, ...], seed: str) -> tuple[bool, str | None]:
    ...
    source = submission.get('lwe.py') if isinstance(submission, dict) else submission
    ...
    try:
        protect_supervisor()
        with LearnerSession({'lwe.py': source}, timeout=RUN_TIMEOUT_SECONDS) as learner:
            module = learner.module()
            failures = []
            for name in phases:
                failures.extend(getattr(check_lwe, name)(module, seed))
        ...
        return False, "lwe.py could not produce the required values within the execution limits"
