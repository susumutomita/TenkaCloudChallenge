...
class LearnerSession:
    ...
    def module(self):
        def function(name):
            return lambda *args: self.call('lwe', name, args)
        return SimpleNamespace(**{name: function(name) for name in ('encode', 'decode', 'centered', 'normalize', 'ring_add', 'ring_sub', 'ring_mul', 'lwe_encrypt', 'lwe_decrypt', 'rlwe_encrypt', 'rlwe_decrypt', 'correspondence', 'survives', 'first_crossing', 'validate_ciphertext')})
