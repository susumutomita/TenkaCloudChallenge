"""Separated hidden verifier for the session-protocol lab."""
...
def _out_of_order_indexes() -> list[int]:
    """Replay the transcript against the documented order and collect the wrong yeses.

    Derived rather than hard-coded so the expected answer follows the seed, and so it
    stays correct if the recorded transcript ever changes shape.
    """
    allowed = {
        ("new", "HELLO"): "greeted",
        ("greeted", "AUTH"): "ready",
    ...
    state = "new"
    wrong: list[int] = []
    for index, row in enumerate(session_transcript(SEED)):
...
def _check_observe(submission: object) -> bool:
    expected = [reported_session(SEED)["sessionId"], "accepted-out-of-order"]
...
def _check_audit(submission: object) -> bool:
    ...
    if not isinstance(value, list) or any(type(item) is not int for item in value):
        return False
    expected = _out_of_order_indexes()
...
from tests.hidden import check_session
private_checker = getattr(check_session, {phase!r})
...
try:
    import session as submission_module
except Exception:
...
if not hasattr(submission_module, "new_session"):
    failures = ["submission does not define new_session()"]
else:
    try:
...
def _check_code(phase: str, submission: object) -> tuple[bool, str]:
    ...
    with tempfile.TemporaryDirectory() as workspace:
        ...
        Path(workspace, "session.py").write_text(submission, encoding="utf-8")
...
def _check_guard(submission: object) -> tuple[bool, str]:
    correct, detail = _check_code(CODE_CHECKPOINT_PHASES["guard"], submission)
    return correct and bool(SEED), detail


def _check_terminal(submission: object) -> tuple[bool, str]:
    correct, detail = _check_code(CODE_CHECKPOINT_PHASES["terminal"], submission)
    return correct and bool(SEED), detail
...
def main() -> None:
    port = int(os.environ.get("VERIFY_PORT", "18541"))
