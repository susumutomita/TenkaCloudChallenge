...
開き、Portal の「証拠を確認」に出た数を貼り、**1 行打って、出た値を貼る**を 10 回繰り返します。
...
3  (a + b) + 2 * x             全部知っている人の式               sum-plain
4  c1 + c2 == ...              一致の確認                        （回答欄なし）
5  x の代わりに huge           15 桁の覆いを、差 1 本で           huge
6  held = c1 + c2; (held, 2x)  相手の手元にあるもの               held
7  held - 2 * x                覆いを外す                        recover
8  候補を数える                 1 つも絞れない                    guesses
9  (c1*c2, ab+(a+b)x, 差)      掛け算は x² を残す                product
10 差 == x * x                 名前のつく前の壁                  （回答欄なし）
```

各行に「この行の意味」が付き、値が合うと「合ったら読む」が開きます。10 行のうち回答欄があるのは
8 行（platform の 1 問あたりの上限）。残り 2 行は、直前の行が内容を担う True を見るだけの行です。
...
3. 1 行目を打ち、出た値を 1 番目の回答欄に貼って提出。その値の 1 文を読む。10 行目まで続ける。
   **回答欄は 1 行の入力欄です。**
...
`make reference-test` が mutation suite を実行します。壊した reference 10 種類（覆いをかぶせ
忘れる、覆いを 1 回しか数えない×3 か所、候補の数え上げで折り返しを忘れる、展開の交差項を
落とす、壁を 2x と比べる、…）を hidden suite が落とすこと、verifier を狙った near-miss 13 種類
