...
from tests.hidden import check_fftdomain
...
def _run_submission(
    ...
    if isinstance(source, dict):
        source = source.get("fftdomain.py")
    ...
    failures = []
    try:
        with LearnerSession({'fftdomain.py': source}, timeout=RUN_TIMEOUT_SECONDS) as session:
            module = session.module()
            for name in phases:
                failures.extend(getattr(check_fftdomain, name)(module, seed))
    except Exception:
        return False, 'The submitted functions did not satisfy the domain and transform rules.'
...
def main() -> None:
    protect_supervisor()
    port = int(os.environ.get("VERIFY_PORT", "18131"))
