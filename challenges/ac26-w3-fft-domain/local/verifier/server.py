...
for name in {phases!r}:
    failures.extend(getattr(check_fftdomain, name)(fftdomain, {seed!r}))
...
def _run_submission(submission: object, phases: tuple[str, ...], seed: str) -> bool:
    ...
    with tempfile.TemporaryDirectory() as workspace:
        (Path(workspace) / "fftdomain.py").write_text(source, encoding="utf-8")
...
_WORKBENCH = PortalEditorSupport(
    ...
    problem_name_en='Does Your Domain Actually Divide?',
