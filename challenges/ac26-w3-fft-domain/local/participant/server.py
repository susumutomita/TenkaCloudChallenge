...
the same image that carried `tests/hidden/check_fftdomain.py` and the process that runs
it. That checker holds `has_order`, the "order exactly n" decision written out from the
definition -- a power that returns 1, and no smaller one that does -- beside `real_omega`
and `naive_omega`, which name an element of each kind for any (p, n). That decision is
exactly what `starter/fftdomain.py` tells the learner nothing else in the image will make
for them, so shipping that file answered the problem rather than hinting at it.
`fixtures/generate.py` left this stage too. `show.py` reads this deployment's public half
-- the field family, the legal orders in each, and the two fixed example domains -- from
the verifier's `GET /public` instead (see show.py, and the VERIFIER_PUBLIC_URL wiring in
../docker-compose.yml). The public tests need no payload at all: every domain they use is
written out in `tests/public/test_fftdomain.py` itself.
"""
...
PORT = int(os.environ.get("WORKBENCH_PORT", "18131"))
