...
class FFTModule:
    """Only returned JSON values cross this boundary; the checker owns the verdict."""
    ...
    def __getattr__(self, name):
        ...
        def invoke(*args):
            result = self.session.call('fftdomain', name, args)
            if type(result) is not dict or type(result.get('ok')) is not bool:
                raise LearnerError('A function must return a response dictionary with boolean ok.')
            if result['ok'] is False:
                if type(result.get('error')) is not str:
                    raise LearnerError('A rejected input must name its error.')
                return result
            if name == 'validate_domain':
                if type(result.get('valid')) is not bool:
                    raise LearnerError('Domain validity must be boolean.')
                return result
            key = {'fft': 'values', 'ifft': 'coefficients', 'interpolate_and_evaluate': 'value'}[name]
            value = result.get(key)
            prime = args[-1]
            values = [value] if key == 'value' else value
            if type(values) is not list or type(prime) is not int or any(type(v) is not int or not 0 <= v < prime for v in values):
                raise LearnerError('Returned values must be integer remainders.')
            if key != 'value' and len(values) != len(args[0]):
                raise LearnerError('The transform must preserve the number of entries.')
            return result
        return invoke
