...
const source = readFileSync(root + '/local/tests/hidden/reader-fftdomain.py', 'utf8');
const files = { 'fftdomain.py': source };
const base = process.env.AC26_WORKBENCH_URL ?? 'http://127.0.0.1:18173';
...
it('edits the real starter, runs public examples, submits all five checkpoints and collapses solved inputs', async () => {
  expect(createHash('sha256').update(source).digest('hex')).toBe('2e7ab7a2e5d811d85d07b33bf38a523bc91875de19a2318da81ae10b854bf49b');
  ...
  const editor = await screen.findByLabelText('fftdomain.py');
  ...
  await screen.findByText(/Small example \/ 小さい例/);
  ...
  expect(screen.getAllByText(codeMessage)).toHaveLength(5);
