...
from participant.protocol import encode, decode, transcript_fields
...
class LearnerSession:
        ...
        while True:
            ...
            if isinstance(value, dict) and value.get('callId') == sequence:
                if 'callback' in value:
                    ...
                    try:
                        reply['value'] = encode(transcript_fields(callbacks[index](*arguments)))
                    except Exception:
    ...
    def module(self):
        return SimpleNamespace(**{name: (lambda *args, _name=name: self.call('capstone', _name, args))
                                  for name in ('scope', 'share', 'run', 'view', 'threshold', 'recover', 'experiment_privacy', 'detects', 'measure', 'evidence')})
