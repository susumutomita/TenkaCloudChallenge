...
Serializable diagnostic extras are preserved when a transcript returns to `view`; opaque extras such as generators are omitted from the value channel. The evaluator still checks only the required transcript fields.
`view` に渡し直すときも、文字列・数・tupleなど転送可能な補足データを保ちます。generatorなど転送できない補足データは省き、採点では必須欄を検査します。
