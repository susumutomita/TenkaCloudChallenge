...
class LearnerSession:
    ...
    def module(self):
        return SimpleNamespace(**{name: (lambda *args, _name=name: self.call('ntt', _name, args))
                                  for name in ('_error', '_is_prime', 'primitive_root_of_unity', '_parse_params', '_parse_list', '_evaluate', 'transform', 'inverse_transform')})
