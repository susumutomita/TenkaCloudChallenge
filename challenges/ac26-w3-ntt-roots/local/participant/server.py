...
the same image that carried `tests/hidden/check_ntt.py` and the process that runs it.
That checker holds `has_order`, "order exactly n" written out from the definition: a
power that returns 1, and no smaller one that does, with the prime factorisation it needs
beside it. It also holds `naive_omega`, named there as "the rule the broken starter uses",
and the `invalid_omega` rejection `inverse_transform` is graded on. `starter/ntt.py` tells
the learner that whatever establishes the order has to be added by them because "nothing
else in the image computes it for you" -- and that sentence was false while the hidden
suite shipped beside it. `fixtures/generate.py` left this stage too; `show.py` reads this
deployment's public half -- the field family, the legal orders in each, and the one worked
transform -- from the verifier's `GET /public` instead (see show.py, and the
VERIFIER_PUBLIC_URL wiring in ../docker-compose.yml). The public tests need no payload at
all: every parameter set they use is written out in `tests/public/test_ntt.py` itself.
"""
...
PORT = int(os.environ.get("WORKBENCH_PORT", "18130"))
