...
`tests/hidden/check_ntt.py` shipped in the learner's own image alongside it. That file
holds `has_order`: "order exactly n" written out from the definition, a power that returns
1 and no smaller one that does, with the prime factorisation it needs beside it. It also
holds `naive_omega`, named there as "the rule the broken starter uses", and it states the
`{"ok": False, "error": "invalid_omega"}` rejection that `inverse_transform` is graded on.
`starter/ntt.py` tells the learner that whatever establishes the order has to be added by
them because "nothing else in the image computes it for you"; that sentence was false
while this file and the hidden checker were in the image with it. `fixtures/generate.py`
shipped there too. That Portal-facing surface now lives in `participant/server.py`, in a
...
def _run_submission(submission: object, phases: tuple[str, ...], seed: str):
    source = submission.get('ntt.py') if isinstance(submission, dict) else submission
    if not isinstance(source, str) or not source.strip() or len(source) > MAX_BODY_BYTES:
        return False, None
    try:
        with LearnerSession({'ntt.py': source}, timeout=RUN_TIMEOUT_SECONDS) as learner:
            failures = []
            for name in phases:
                failures.extend(getattr(check_ntt, name)(learner.module(), seed))
