# Watch the team server — CloudWatch basics
...
Select the prepared EC2 CPU metric → configure a named alarm → distinguish valid configuration from a tested alarm firing.
...
One t3.micro EC2 with 8 GiB gp3, isolated VPC/subnet/security group, one participant-created CPU alarm, checker/cleanup Lambdas and one-day logs. No public IPv4, internet gateway, NAT gateway or SNS subscription is needed.
...
DescribeAlarms checks exact InstanceId, CPUUtilization, Average, 300 seconds, greater than 80 and one evaluation. Notification/recovery actions are excluded (IAM also prohibits alarm actions). INSUFFICIENT_DATA initially is accepted: actual triggering and email delivery are not tested.
