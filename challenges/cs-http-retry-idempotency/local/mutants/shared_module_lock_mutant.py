"""Author-only mutant: check-then-insert guarded by a lock stashed on the sqlite3 module.

Every copy of the program that runs inside one interpreter finds the same lock object
on the shared `sqlite3` module, so this serialized the old "two module copies in one
process" round completely. Worker processes each have their own `sqlite3` module and
therefore their own lock: the check-then-insert race is untouched, and the round must
kill it.
"""
import hashlib, json, sqlite3, threading

_LOCK = getattr(sqlite3, "_tenka_gateway_lock", None)
if _LOCK is None:
    _LOCK = threading.Lock()
    sqlite3._tenka_gateway_lock = _LOCK
