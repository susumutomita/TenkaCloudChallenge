"""Author-only mutant: BEGIN IMMEDIATE placed after the absent decision.

The write turn is taken only once two threads have already read "absent".
"""
...
def handle_request(db_path, idempotency_key, request):
    ...
    try:
        ...
        connection.execute("BEGIN IMMEDIATE")   # too late: the absent decision is already made
