"""Author-only mutant: the check-then-insert a participant has after replay/bind.

Reads the receipt in autocommit mode, sees nothing, inserts. This passed the old
eight-thread test on every run; the interleave scheduler must kill it every time.
"""
...
def handle_request(db_path, idempotency_key, request):
    ...
    try:
        ...
        connection.execute("INSERT INTO receipts(key, fingerprint, status, body) VALUES (?, ?, ?, ?)",
                           (key, fingerprint, 201, json.dumps(body, sort_keys=True, separators=(",", ":"), ensure_ascii=False)))
        connection.commit()
    finally:
        connection.close()
