"""Author-only honest shape (must pass): `with connection:` around one serialized decision.

The context manager commits on success and rolls back on an exception; the write turn
is taken with `BEGIN IMMEDIATE` inside it, before the read.
"""
...
def handle_request(db_path, idempotency_key, request):
    ...
    try:
        ...
        with connection:                        # commit on success, rollback on an exception
