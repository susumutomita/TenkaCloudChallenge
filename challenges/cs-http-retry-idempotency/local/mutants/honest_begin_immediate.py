"""Author-only honest shape (must pass): the statement's recipe (a) applied to the starter.

`BEGIN IMMEDIATE` before the receipt read, in the starter's legacy isolation mode, with
the receipt table exactly as the statement's example names it.
"""
...
def handle_request(db_path, idempotency_key, request):
    ...
    try:
        ...
        connection.execute(
            "CREATE TABLE IF NOT EXISTS receipts (key TEXT NOT NULL, fingerprint TEXT NOT NULL, status INTEGER NOT NULL, body TEXT NOT NULL)"
        )
        connection.execute("BEGIN IMMEDIATE")   # from here to commit() is one transaction
