...
_WORKBENCH = PortalEditorSupport(
    ...
    seed=SEED,
    problem_id='ac26-w5-pbs-homnand',
    problem_name='暗号文のまま関数を引き、入力の鍵へ戻す',
    problem_name_en='Look up a function on a ciphertext, and return to the input key',
    description='まずpipeline.pyのlookup_accumulatorを実装し、lutを提出します。暗号文のまま表を読み、誤差を更新する部品を順につなぎます。',
    description_en="First implement lookup_accumulator in pipeline.py and submit lut. Then connect encrypted table evaluation and error refresh.",
    checkpoint_labels={'lut': '関数を多項式に書く', 'domain': '回転の単位へ移す', 'rotate': '暗号化されたまま回す', 'relabel': '鍵の所属を付け替える', 'evaluate': '暗号文のまま関数を引く', 'refresh': '何が起きたかを記録する', 'nand': 'ゲートを組んで真理値表をそろえる', 'transfer': '未見の設定と丸め誤差の反例で確かめる'},
    checkpoint_labels_en={'lut': 'Write the function into a polynomial', 'domain': 'Move to the units of rotation', 'rotate': 'Turn it while it stays encrypted', 'relabel': 'Move it between key domains', 'evaluate': 'Look up a function on a ciphertext', 'refresh': 'Record what actually happened', 'nand': 'Build the gate and complete the truth table', 'transfer': 'Check unseen settings and construct rounding evidence'},
