...
class LearnerSession:
    ...
    def module(self):
        def function(name):
            return lambda *args: self.call('pipeline', name, args)
        return SimpleNamespace(**{name: function(name) for name in ('lookup_accumulator', 'to_rotation_domain', 'blind_rotate', 'extract', 'switch', 'bootstrap', 'pipeline_trace', 'output_noise_bound', 'correctness_bound', 'refresh_report', 'nand_combine', 'homomorphic_nand', 'rounding_counterexample')})
