...
the single Docker stage a learner's own `make build` produced -- so `fixtures/generate.py`
and `tests/hidden/check_recover.py` shipped in the learner's own image alongside it.
`fixtures/generate.py` holds `audit_log`, which returns `victim_secret` and
...
explaining what has to go into the hash and why. That Portal-facing surface now lives in
...
The supplied half a learner does need -- the group, the Fiat-Shamir challenge, the
signing routine, and the truncated generator the `collision` checkpoint measures -- moved
to `participant/schnorr.py`, which both images carry. `GET /public` below is what
`show.py` reads instead of importing `fixtures.generate`.
...
# `participant/schnorr.py`, and `starter/recover.py` tells the learner to call it by name
# -- the `collision` checkpoint is a measurement of `truncated_nonce`. The guard below
# takes the problem root off `sys.path`, so without preloading it here a submission's own
# top-level `from participant.schnorr import ...` would not resolve and every checkpoint
# would fail on an import error. It stays in `sys.modules` across the guard on purpose;
# `fixtures` and `tests` do not.
import participant.schnorr  # noqa: F401
...
for name in {phases!r}:
    failures.extend(getattr(check_recover, name)(recover, {seed!r}))
...
def _run_submission(
    ...
    if isinstance(source, dict):
        source = source.get("recover.py")
    if not isinstance(source, str) or not source.strip():
        return False, None
    if len(source) > MAX_BODY_BYTES:
        return False, None
    with tempfile.TemporaryDirectory() as workspace:
        (Path(workspace) / "recover.py").write_text(source, encoding="utf-8")
...
def _unwrap_submission(checkpoint_id: str, submission: object) -> object:
    ...
    shape as ac26-w3-fft-domain's and ac26-w4-commit-open's verifiers, for the same
    reason.
...
def main() -> None:
    port = int(os.environ.get("VERIFY_PORT", "18152"))
    ...
    #
    # The restriction that matters is in docker-compose.yml: since Issue 543 option B2
    # this service publishes no port at all and sits on an internal-only network, so the
    # only client that can reach it is the Workbench container beside it.
