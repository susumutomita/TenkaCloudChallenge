...
class LearnerSession:
    ...
    def call(self, module, function, args):
        if time.monotonic() >= self.deadline:
            raise LearnerError('Function evaluation timed out.')
        sequence = secrets.token_hex(16)
        callbacks = {i: arg for i, arg in enumerate(args) if callable(arg)}
        from participant.schnorr import Group, truncated_nonce
        groups = [arg for arg in args if isinstance(arg, Group)]
        def generator(seed, secret, message, group):
            # Reassociate an inertly decoded group with this call's issued input.
            for supplied in groups:
                if (group.p, group.a, group.b, group.n, group.generator.x, group.generator.y) == (supplied.p, supplied.a, supplied.b, supplied.n, supplied.generator.x, supplied.generator.y):
                    group = supplied
                    break
            return truncated_nonce(seed, secret, message, group)
        callbacks[-1] = generator
        wire_args = [ ['callback', i] if i in callbacks else encode(arg) for i, arg in enumerate(args)]
        request = {'callId': sequence, 'module': module, 'function': function,
                   'args': wire_args}
        try:
            self._send(request)
        ...
        while True:
            ...
            try:
                ...
                value = None
            if isinstance(value, dict) and value.get('callId') == sequence:
                if 'callback' in value:
                    ...
                    try:
                        reply['value'] = encode(callbacks[index](*arguments))
                    except Exception:
                        reply['error'] = 'ValueError'
                    self._send(reply)
                    continue
                if value.get('error') == 'MalformedRecord':
                    raise MalformedRecord('The submitted function refused the record.')
                if 'error' in value:
    ...
    def module(self):
        return SimpleNamespace(MalformedRecord=MalformedRecord, **{name: (lambda *args, _name=name: self.call('recover', _name, args))
                                  for name in ('parse_record', 'accepts', 'find_reuse', 'recover_secret', 'confirms', 'attack_log', 'collision_experiment', 'safe_nonce', 'repair_witness')})
