...
_WORKBENCH = PortalEditorSupport(
    ...
    seed=SEED,
    problem_id='ac26-w3-nonce-reuse',
    problem_name='乱数再利用から署名の秘密鍵を復元する',
    problem_name_en='Recover a signing key from nonce reuse',
    description='署名サービスの audit log には message、public key、R、z がある。秘密鍵は無い。有効で同じ公開鍵とRを持ち、質問eが異なる2記録から秘密鍵を復元します。',
    description_en="A signing service's audit log holds the message, the public key, R and z. It does not hold the secret key. Recover the key from two valid records with the same public key/R and different challenges.",
