...
class LearnerSession:
    ...
    def module(self):
        def function(name):
            return lambda *args: self.call('stack', name, args)
        return SimpleNamespace(**{name: function(name) for name in ('carried', 'contract_violations', 'first_failure', 'underwrites', 'property_map', 'counterexample', 'repair', 'select')})
