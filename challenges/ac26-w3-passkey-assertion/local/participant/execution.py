...
class LearnerSession:
    ...
    def module(self):
        return SimpleNamespace(**{name: (lambda *args, _name=name: self.call('assertion', _name, args))
                                  for name in ('_decode', '_point_add', '_scalar_mul', '_ecdsa_verify', 'signed_message', 'verify_signature', 'user_verified', 'find_signed_without_user_verification', '_context_failure', 'verify_assertion')})
