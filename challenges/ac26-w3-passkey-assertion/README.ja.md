...
## Issue #716: ヒントと提供済み処理の確認

3チェックを日英の3段ヒント（仕組み・小例・実装と提出、各2点）へ変更しました。参加者向け資料だけを読む独立読者から、手作業のbase64urlデコード、origin/typeの意味、提供済み文脈チェックの例外停止が指摘されました。用意された_decodeと0始まりの番号を説明し、文脈の意味を補い、配列など辞書以外のJSONをmalformed-assertionで拒否します。署名の欠落は署名検査で拒否する手順を明示しました。

make install・make agent-gate（116教材）、内部回帰2件を通過。内部回帰は不正なJSON形状と署名欠落を検証します。独立読者は非公開資料を読んでいません。ライブ提出は未実施です。


レビュー追補：署名欠落を signature と enforce-uv の採点器に追加し、辞書の直接参照で例外になる2種の変異を不合格にしました。reference-test が使う実行器で内部回帰テストも検出します。Python直接実行で回帰2件と変異8種の拒否、カタログ検証を確認しました。この追補ではDocker・実Portalは未実施です。
