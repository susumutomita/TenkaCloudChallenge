...
天下クラウド株式会社、 ネットワーク班 4 日目。 前任の SRE が置いていった社内 HTTPS エンドポイントに、 誰も繋げなくなった。

> CTO 曰く: 「踏み台 (`relay`) からアプリ (`app`) の :443 に `curl` すると、 TLS の握手 (handshake) でコケる。 証明書が合わないとか何とか。 前任の SRE は『証明書は自動で作る仕組みにした』って言ってた。 仕組みは動いてる。 なのに弾かれる」
...
3. **なぜ名前が食い違うのか** を SSM の設定から追う:
   ...
   - `app` が証明書に焼く名前は SSM から来る:
     ...
     # -> legacy.kato.example   (前任の SRE の置き土産 ── 間違った名前)
     ```
