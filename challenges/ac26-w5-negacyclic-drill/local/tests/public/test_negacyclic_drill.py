...
import negacyclic_drill as drill
from participant.exercise import ROW_ARGS,EXAMPLE,EXAMPLE_EXPECTED,call_row
from participant.model import valid_failure,valid_repair
...
def main():
    ...
    if not rows:
        ...
        return 1
    passed=True
    print('== small example: p=16, n=8, total noise=1 ==')
    for row in rows:
        try:
            got=call_row(drill,row,EXAMPLE)
            if row in ('constants','margin'):
                same=(valid_failure if row=='constants' else valid_repair)(EXAMPLE,got)
