...
from fixtures.generate import GRADED,normalize_answer,setting,valid_failure,valid_repair
    ...
    for context,inputs,expected in [('example',EXAMPLE,EXAMPLE_EXPECTED),('deployment',fixture['public'],fixture['expected'])]:
        for row in GRADED:
            try:
                ...
                valid=(valid_failure if row=='constants' else valid_repair)(inputs,actual) if row in ('constants','margin') else actual is not None and actual==normalize_answer(row,expected[row])
                if not valid:
