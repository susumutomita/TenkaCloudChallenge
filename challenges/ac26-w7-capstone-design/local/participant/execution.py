...
class LearnerSession:
    ...
    def module(self):
        def function(name):
            return lambda *args: self.call('design', name, args)
        return SimpleNamespace(**{name: function(name) for name in
                                  ('classify_assets', 'required_properties', 'compare_alternatives',
                                   'select_primitive', 'architecture', 'attack_plan',
                                   'property_matrix', 'revise')})
