...
天下クラウド株式会社、 ネットワーク班 3 日目。 前任の SRE が残した監視ジョブは、 private サブネットの管理ノード `core` から「外」に問い合わせて結果を持ち帰る ── はずだった。

> CTO 曰く: 「`core` が外に出られない。 ライセンス確認も、 アップデートも、 何も取りに行けない。 IPv4 が枯れてから NAT で逃がす設計にしたって前任の SRE は言ってた。 踏み台 (`relay`) は普通に外に出られるのに、 奥の `core` だけが沈黙してる」
