...
`circuit.py` against the player's own witness and repair, in their own terminal; by
the time a string reaches here the only question left is whether it is this
deployment's flag. That is why there is no submission sandbox in this file --
there is no submission to sandbox.
"""
        ...
        self._respond(
            200,
            {
                ...
                "message": (
                    "Flag accepted."
                    if correct
                    else "That is not the flag for this deployment. `circuit flag` prints it "
                    "once both stages are cleared."
                ),
            },
        )
