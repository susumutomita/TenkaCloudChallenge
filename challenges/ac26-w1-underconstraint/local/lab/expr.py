...
A repair is submitted as the polynomial that must come out zero:

    revoked*ok                  ->  the residual `revoked * ok`
    revoked*inv + ok - 1        ->  the residual `revoked * inv + ok - 1`
    revoked*inv + ok = 1        ->  the same thing; `=` is subtraction

Grammar, smallest thing that can express a rank-1 constraint:
...
No division: a constraint system has no division either, and the whole subject of
this problem is that "the inverse" is a signal the prover supplies rather than an
operation the circuit can perform.
"""
