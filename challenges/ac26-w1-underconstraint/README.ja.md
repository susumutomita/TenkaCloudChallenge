...
Participant Portal で問題を起動すると、`policy.py` のエディタが問題文と同じ画面に表示されます。
証拠の確認、編集、公開テスト、build / audit / exploit / repair / mutation-transfer の提出まで Portal 内で
完結します。root-cause は Portal に表示される形式の JSON を自分で組み立てて回答欄へ入力します。
