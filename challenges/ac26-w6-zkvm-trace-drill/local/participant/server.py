...
_WORKBENCH = PortalEditorSupport(
    ...
    checkpoint_labels={'exact': '普通に足した合計', 'trace': '機械の途中経過を記録する', 'overflow': 'どの足し算で桁があふれたか', 'decision': '二つの承認結果を比べる', 'exploit': '不正な承認を見つける', 'predicate': '足りない条件の反例を選ぶ', 'tamper': '報告を計算し直して確かめる', 'binding': '別のプログラムでしか通らない入力を作る'},
    checkpoint_labels_en={'exact': 'Add without wrapping', 'trace': 'Record each machine step', 'overflow': 'Locate each overflow', 'decision': 'Compare the two acceptance decisions', 'exploit': 'Identify an improper acceptance', 'predicate': 'Find counterexamples to an incomplete condition', 'tamper': 'Recompute rather than trust a report', 'binding': 'Construct an input that works only for another program'},
