/**
 * db-a8-delete-vacuum — local-play container entrypoint (Node half).
 *
 ...
 * server.mjs) — telemetry.events starts from a FIXED seed
...
import { createPgGraderClient, RETENTION_CUTOFF, SEEDED_RECENT_ROWS } from "./pg-client.mjs";
...
<title>db-a8-delete-vacuum</title></head>
<body style="font-family:system-ui;max-width:34rem;margin:3rem auto;line-height:1.6">
<h1>DELETE したのに disk が減らない</h1>
<p>このドリルは画面の上では完結しません。Portal のターミナルを開き、以下で接続してください。</p>
<pre>psql -U participant -d drill</pre>
<p>ゴール: retention cutoff (${RETENTION_CUTOFF}) より古い telemetry.events の行
だけを DELETE し、table size がすぐには縮まないこと・n_dead_tup が増えること
を観測してから、VACUUM (autovacuum は無効) を実行して dead tuple が回収され
ることを確認する。cutoff 以降の ${SEEDED_RECENT_ROWS} 行はこのドリルに一切
関わらない — 無傷のまま残さなければならない。</p>
...
const info = createServer(async (request, response) => {
  ...
  if (request.method === "GET" && url.pathname === "/status") {
    try {
      const [rowCounts, eventStats, deleteLog, relationSizeBytes] = await Promise.all([
        client.rowCounts(),
        client.eventStats(),
        client.deleteLog(),
        client.relationSizeBytes(),
      ]);
      return send(response, 200, {
        retentionCutoff: RETENTION_CUTOFF,
        rows: rowCounts,
        stats: eventStats,
        deleteLogEntries: deleteLog.length,
        deleteLogTotalRowsDeleted: deleteLog.reduce((sum, e) => sum + e.rowsDeleted, 0),
        relationSizeBytes,
      });
    } catch {
