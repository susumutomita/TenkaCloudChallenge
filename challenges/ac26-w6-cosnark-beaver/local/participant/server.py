...
_WORKBENCH = PortalEditorSupport(
    ...
    description='まずprover.pyでmultiplication_planを実装し、planを提出します。分けて持った秘密の数を掛ける方法と、記録だけでは隠し方の安全性を確認しきれない理由を学びます。',
    description_en='First implement multiplication_plan in prover.py and submit plan. Learn to multiply secrets held in shares and why records alone cannot establish that masking is safe.',
    checkpoint_labels={'plan': '通信する前に、 何回話すかを数える', 'triple': 'mask は一度きり', 'masks': '共有値を覆う。 まだ誰も話さない', 'open': '2 つの値を、 1 度の待ち合わせで', 'product': '4 つの項のうち、 1 つだけ性質が違う', 'artifact': '次の段が受け取れる形にする', 'audit': '何が公開されたのかを、 記録から測る', 'transfer': '未見の設定と、マスク相殺の反例で確かめる'},
    checkpoint_labels_en={'plan': 'Count the talking before you do any', 'triple': 'Use each mask only once', 'masks': 'Cover the shared values. Nobody talks yet', 'open': 'Two values, one wait', 'product': 'Four terms, one of them unlike the others', 'artifact': 'The shape the next stage consumes', 'audit': 'Measure what was published, from the record', 'transfer': 'Check unseen settings and a mask-cancellation counterexample'},
