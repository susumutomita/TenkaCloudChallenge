...
from participant.mpc import ParticipantRuntime, CrossPartyRead, TripleMisuse
...
class LearnerSession:
            ...
            while True:
                ...
                if isinstance(value, dict) and value.get('callId') == self.sequence:
                    if 'runtime' in value:
                        ...
                        except (ValueError, TypeError, CrossPartyRead, TripleMisuse) as error:
    ...
    def module(self):
        return SimpleNamespace(**{name: (lambda *args, _name=name: self.call(_name, args)) for name in
                                  ('multiplication_plan', 'reserve_fresh_triple', 'masked_operands',
                                   'open_masks', 'shared_product', 'prove_product', 'proof_artifact',
                                   'privacy_audit', 'mask_cancellation_witness')})
