"""Parameters, the supplied sharing layer, the instrumented runtime, and the triple dealer.
...
Two problems are **supplied**. `ac26-w2-beaver-mul` built the multiplication protocol itself,
and `ac26-w6-cosnark-linear` built the prover's linear layer and the runtime that proves it
stayed local. This problem is what happens when those meet: the one place in a co-SNARK's
prover row where the parties have to talk.
...
sharings are handed to you already built. `C` is a product of two **shared** values, and the
product of the sums is not the sum of the products, so no arrangement of local operations
produces it. One round of communication is the price, and Beaver's trick is what makes it
exactly one.

## The trick, and where the cost actually went
...
nothing about either -- provided the mask is used **once**. The whole security of the step is
that sentence, which is why `reserve_triple` will not hand the same triple out twice.

The cost did not disappear; it moved into preprocessing. Producing triples is the expensive,
input-independent part, and the online phase is one round however big the circuit's depth of
independent multiplications is, because every multiplication in a layer batches into it.

## `d * e` is public, and public constants are not added to every share

`d*e` is a number everybody knows. Week 2's lesson applies unchanged: adding a public constant
to every party's share gives a sharing of `value + parties * constant`. Exactly one party folds
it in. `add_public` is local and does what it is told; which party calls it is the participant's
decision and the classic way to be wrong by a factor nobody notices at two parties.
