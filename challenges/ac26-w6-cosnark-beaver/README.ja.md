...
PR #831 追加対応：最終構成では実 Runtime の読取数を、成功したローカル演算の必要読取数と比較します。基底クラスの ParticipantRuntime から直接呼び出す場合も計測対象です。作者回帰 8 件で直接読取・基底呼出しと別の正常な構成を確認します。これは提供 API の観測であり、Python の安全な隔離環境ではありません。無料の式・API 契約を維持し、最終の小例は完成した構成ではなく失敗する候補の検算へ変更しました。
