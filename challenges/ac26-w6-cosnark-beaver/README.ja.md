...
`make reference-test`はmutation.pyから構成回帰9件と32変異検査を実行します。ホストとDockerの作者検査ですべて成功。参加者資料だけの独立読解で4件の説明不整合を見つけ修正しました。実Workbenchのハンドラメソッドでstarter取得・提出準備・公開9テスト成功を確認。sandboxでHTTP起動できず、ブラウザ操作・配備後採点・デプロイは未実施です。カタログ116件有効。


PR #831 追加対応：最終構成では採点側が所有するクロージャで値の読取を記録し、成功したローカル演算の必要読取数と比較します。基底クラスの ParticipantRuntime からの直接呼び出しも計測し、公開の Runtime.reads を戻しても記録は消えません。作者回帰 9 件で直接読取・基底呼出しと別の正常な構成を確認します。これは提供 API の観測であり、Python の安全な隔離環境ではありません。無料の式・API 契約を維持し、最終の小例は完成した構成ではなく失敗する候補の検算へ変更しました。
