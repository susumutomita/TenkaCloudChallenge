...
```bash
make inspect            # your setting, your row, your triple, and what the log records
...
`make inspect` opens two scratch sharings under one round id — one masked by a reserved
triple share, one not — and prints both records. Neither touches `A` or `B`. It is worth
looking at before you decide the two are the same operation.
