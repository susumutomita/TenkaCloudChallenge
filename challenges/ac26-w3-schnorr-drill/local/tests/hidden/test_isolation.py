...
@unittest.skipUnless(os.environ.get('SCHNORR_WORKBENCH_URL'), 'requires the dedicated live Compose workbench')
...
'''
        ...
        result = self.call('/api/test', {'files': {'schnorr_drill.py': starter['schnorr_drill.py'] + probe}})
    ...
    def test_private_key_route_is_not_a_public_proxy_route(self):
        for path in ('/workbench-key', '/api/workbench-key', '/verify/workbench-key'):
            for payload in (None, {'checkpointId': 'field-inv', 'submission': 'x'}):
