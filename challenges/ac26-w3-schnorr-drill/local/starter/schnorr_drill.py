...
All horizontal/vertical calculations use division remainders by prime p
(Python % p, also written mod p).
The order n counts additions of G until O; response counts use % n instead.
x is the secret, r a one-time random number, and e the question number.
P=x*G is the public point made by adding G x times.
...
def add_points(G: tuple, Q: tuple, p: int) -> tuple:
    """Field 2 — G + Q using the point-addition formula in the statement. Returns (x3, y3)."""
    return None


def double(G: tuple, p: int, a: int) -> tuple:
    """Field 3 — 2G using the tangent: the line following the curve at G. Returns (x3, y3)."""
    return None
...
def nonce_reuse(s1: int, s2: int, e1: int, e2: int, n: int) -> int:
    """Field 7 — the secret behind two responses using the same one-time random number (nonce)."""
    return None


def transfer(P2: tuple, ef: int, G: tuple, p: int, a: int) -> tuple:
    """Field 8: construct [Rx,Ry,s] for the already visible question number ef.
