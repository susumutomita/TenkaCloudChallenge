...
if phases:
    failures = []
    for name in phases:
        failures.extend(getattr(check_schnorr_drill, name)(schnorr_drill, {seed!r}))
else:
    failures = check_schnorr_drill.run(schnorr_drill, {seed!r})
...
def _run_submission(submission: object, phases: tuple[str, ...], seed: str) -> bool:
    """Run the hidden suite against a `schnorr_drill.py` in a throwaway workspace.
    ...
    if isinstance(source, dict):
        source = source.get("schnorr_drill.py")
    if not isinstance(source, str) or not source.strip():
        return False
    if len(source) > MAX_BODY_BYTES:
        return False
    with tempfile.TemporaryDirectory() as workspace:
        (Path(workspace) / "schnorr_drill.py").write_text(source, encoding="utf-8")
...
_WORKBENCH = PortalEditorSupport(
    ...
    problem_name='1 行打って、出た値を貼る — 有限体から nonce 再利用まで 12 行',
    problem_name_en='Type one line, paste the value — twelve lines from the field to nonce reuse',
