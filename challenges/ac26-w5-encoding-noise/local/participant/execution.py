...
class LearnerSession:
    ...
    def module(self):
        def function(name):
            return lambda *args: self.call('encoding', name, args)
        return SimpleNamespace(**{name: function(name) for name in ('validate_params', 'encode', 'centered', 'add_noise', 'decode', 'success_interval', 'first_failure', 'counterexample')})
