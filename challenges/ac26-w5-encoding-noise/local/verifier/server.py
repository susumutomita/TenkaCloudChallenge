...
def _run_submission(submission: object, phases: tuple[str, ...], seed: str) -> tuple[bool, str | None]:
    ...
    source = submission.get('encoding.py') if isinstance(submission, dict) else submission
    ...
    try:
        protect_supervisor()
        with LearnerSession({'encoding.py': source}, timeout=RUN_TIMEOUT_SECONDS) as learner:
            module = learner.module()
            failures = []
            for name in phases:
                failures.extend(getattr(check_encoding, name)(module, seed))
        ...
        return False, "encoding.py could not produce the required values within the execution limits"
