...
`tests/hidden/check_encoding.py` shipped in the learner's own image alongside it, and
every checkpoint is graded by running that suite. `fixtures/generate.py` shipped there
too, and it implements `encode`, `centered`, `decode`, `success_interval` and
`first_failure` under the exact five names `starter/encoding.py`'s own stubs ask the
learner to write. That Portal-facing surface now lives in `participant/server.py`, in a
...
def main() -> None:
    port = int(os.environ.get("VERIFY_PORT", "18145"))
