...
| `local/config/app.json` | アプリの設定 ── **自分のファイル**。 コンテナには読み取り専用でマウントされる |
...
4. 板はまだ投稿を受け付けておらず、 そのことが板自身に書いてある。 それを決めている設定は自分のチェックアウト側にある。
...
6. 終わったら設定を戻す。 `git checkout -- challenges/stackstack-onboarding/local/config/`
