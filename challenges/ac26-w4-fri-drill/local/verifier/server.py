...
if phases:
    failures = []
    for name in phases:
        failures.extend(getattr(check_fri_drill, name)(fri_drill, {seed!r}))
else:
    failures = check_fri_drill.run(fri_drill, {seed!r})
...
def _run_submission(submission: object, phases: tuple[str, ...], seed: str) -> bool:
    """Run the hidden suite against a `fri_drill.py` in a throwaway workspace.
    ...
    if isinstance(source, dict):
        source = source.get("fri_drill.py")
    if not isinstance(source, str) or not source.strip():
        return False
    if len(source) > MAX_BODY_BYTES:
        return False
    with tempfile.TemporaryDirectory() as workspace:
        (Path(workspace) / "fri_drill.py").write_text(source, encoding="utf-8")
...
_WORKBENCH = PortalEditorSupport(
    ...
    problem_name='1 行打って、出た値を貼る — FRI の折り畳みとクエリ検査',
    problem_name_en='Type one line, paste the value — FRI folding and the query check',
...
def main() -> None:
    port = int(os.environ.get("VERIFY_PORT", "18135"))
