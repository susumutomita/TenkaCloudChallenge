...
-- the same image that carried `tests/hidden/check_auditor.py` and the process that runs
it. All seven checkpoints are graded by running that suite against the submitted file,
and its `_expected_index` and `_leaks` state the decision rule `first_violation` exists
to make a learner derive, event kind by event kind. `fixtures/generate.py` left this
stage too: its `TRUTH` names the verdict for each of the seven programs by id, and
`program()`'s comments name the violation each one carries. `show.py` and the public
tests read this deployment's public half -- the specification and the one clean run's
trace -- from the verifier's `GET /public` instead (see show.py and
tests/public/test_auditor.py, and the VERIFIER_PUBLIC_URL wiring in
../docker-compose.yml).
"""
...
PORT = int(os.environ.get("WORKBENCH_PORT", "18098"))
...
_WORKBENCH = PortalEditorSupport(
    ...
    code_checkpoints=('allowed-opens', 'opened-secret', 'cross-party', 'log-leak', 'transcript', 'repair', 'mutation'),
    checkpoints=('allowed-opens', 'opened-secret', 'cross-party', 'log-leak', 'transcript', 'repair', 'mutation'),
