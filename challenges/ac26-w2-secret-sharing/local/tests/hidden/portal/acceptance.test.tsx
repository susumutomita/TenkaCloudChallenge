...
const files=Object.fromEntries(['sharing.py'].map(name=>[name,readFileSync(reader+'reader-'+name,'utf8')]));
const base=process.env.AC26_WORKBENCH_URL ?? 'http://127.0.0.1:18095';
...
it('submits the four code fields and public hand calculation through real Portal components and local API',async()=>{
 const user=userEvent.setup();show(flags);await screen.findByLabelText('sharing.py');
 expect(screen.getAllByText('This checkpoint submits the current source from the editors above.')).toHaveLength(4);
 const answer=readFileSync(reader+'threshold-hand.json','utf8').trim();
 await user.click(screen.getByLabelText(/必要な人数と2通りの補完を示す/));
 ...
 for(const call of mocks.prepareWorkbench.mock.calls){expect(call[3]).toEqual(files);expect(call[4]).toEqual({'threshold':answer});}
