/**
 * db-challenge-slow-query — local-play container entrypoint (Node half).
 *
 ...
 *          page just confirms the Challenge is up and restates the slow
 *          query (it does NOT explain why it's slow — see metadata.json's
 *          instructions for the same non-spoiler framing).
 ...
 * target lookup EXACTLY ONCE against the freshly seeded database (red
 * herring index already in place, no participant fix yet) and records its
 * buffer cost in `grading.baseline_buffers` — the number every later
 * /verify call compares against. Same pattern as
 * db-a2-index-tradeoff/local/app/server.mjs.
 */
...
console.log(`[db-challenge-slow-query] baseline buffers for the target lookup: ${baseline}`);
...
const TARGET_QUERY_TEXT = `select id, status, total_cents, created_at
...
const info = createServer(async (request, response) => {
  ...
  if (request.method === "GET" && url.pathname === "/status") {
    try {
      const [leadsIndex, plan, storedBaseline] = await Promise.all([
        client.ordersCustomerIdLeadsAnIndex(),
        client.explainTargetQuery(),
        client.baselineBuffers(),
      ]);
      return send(response, 200, {
        targetQuery: TARGET_QUERY_TEXT,
        someIndexLeadsWithCustomerId: leadsIndex,
        currentPlanNodeTypes: [...plan.nodeTypes],
        currentSeqScanRelations: [...plan.seqScanRelations],
