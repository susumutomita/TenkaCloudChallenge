...
This image carries public fixtures, tests, and the starter only. ``/verify`` is a small reverse
proxy to the separately built verifier container, so the Portal keeps one loopback URL while
author artifacts (reference implementation, hidden checks, mutation suite) stay out of the
participant image.
"""
...
PORT = int(os.environ.get("WORKBENCH_PORT", "18620"))
VERIFIER_URL = os.environ.get("VERIFIER_URL", "http://verifier:18621/verify")

MAX_BODY_BYTES = 256 * 1024
MAX_OUTPUT_BYTES = 64 * 1024
RUN_TIMEOUT_SECONDS = 12
REQUEST_TIMEOUT_SECONDS = 15
SUBMISSION_FILES = ("migration.py",)
CHECKPOINTS = (
    "inventory",
    "preserve-identity",
    "publish-records",
...
CODE_CHECKPOINTS = frozenset(
    ("preserve-identity", "publish-records", "least-privilege", "deadline-retry", "verify-renewal")
)
CHECKPOINT_LABELS = {
    "inventory": "inventory — 証明書・SAN・依存・DNS owner を数え上げる",
    "preserve-identity": "preserve-identity — 証明書 ARN を差し替えない",
    "publish-records": "publish-records — 全ドメインの正しい CNAME を正しい zone へ publish する",
    "least-privilege": "least-privilege — DNS 書込み権限を必要な zone/record に限定する",
    "deadline-retry": "deadline-retry — 72時間の締切と retry/abort を状態機械として扱う",
    "verify-renewal": "verify-renewal — 全ドメインの検証完了と更新資格を外部状態で確認する",
}
...
def config_payload() -> dict[str, object]:
    return {
        ...
        "name": "証明書のARNは変えずに、検証方式だけを乗り換える",
        "description": "既存ACM証明書のARNを変えずに、email validationからDNS validationへ移行する計画を、オフラインのローカルsimulatorで実装する。",
...
runpy.run_path({root!r} + "/tests/public/test_migration.py", run_name="__main__")
"""
...
def run_public_tests(seed: str, files: object) -> dict[str, object]:
    ...
    if sources is None:
        return {"passed": False, "output": "migration.py must be a non-empty Python file."}
    with tempfile.TemporaryDirectory() as workspace:
        Path(workspace, "migration.py").write_text(sources["migration.py"], encoding="utf-8")
...
def prepare_submissions(seed: str, files: object) -> dict[str, object]:
    sources = _submission_sources(files)
    if sources is None:
        return {"ok": False, "output": "migration.py must be a non-empty Python file."}
    source = sources["migration.py"]
    return {
        "ok": True,
        "submissions": {checkpoint: source for checkpoint in CODE_CHECKPOINTS},
    }
...
def main() -> None:
    # Containers must bind all interfaces so Docker port publishing can reach the process.
    # Compose exposes it only on host loopback (127.0.0.1).
