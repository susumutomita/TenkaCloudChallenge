...
''')
    ...
    def test_system_v_and_posix_message_queues_cannot_survive_a_submission(self):
        ...
        source = field_source('''    def normalize(self, arguments):
...
''')
        for attempt in range(64):
            ...
            try:
                with LearnerSession(source) as learner:
                    results = learner.call('field', 'normalize', [[name, mq_unlink_number]], 97)
    ...
    def test_posix_shared_memory_and_named_semaphore_creation_use_denied_file_opens(self):
        ...
        source = field_source('''    def normalize(self, names):
        lib = ctypes.CDLL(None, use_errno=True)
...
''')
        for attempt in range(64):
            ...
            try:
                with LearnerSession(source) as learner:
                    self.assertEqual(learner.call('field','normalize',[names],97), [errno.EPERM]*2)
...
''')
    ...
    def test_partial_output_and_blocked_input_respect_one_deadline(self):
        ...
        while True: pass
''')
        # Start the interpreter before measuring the same 200ms I/O deadline;
        # host load must not turn this into an unrelated startup timeout test.
        with LearnerSession(sources) as learner:
            start=time.monotonic()
            learner.deadline=start+.2
...
''')
        ...
        with LearnerSession(sources) as learner:
            learner.deadline=time.monotonic()+.2
