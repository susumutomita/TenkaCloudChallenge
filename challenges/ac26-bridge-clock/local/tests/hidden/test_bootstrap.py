...
class Bootstrap(unittest.TestCase):
    def test_seals_keep_the_existing_verifier_contract(self):
        for seed in ('bootstrap-example-a', 'bootstrap-example-b'):
            with patch.object(verifier, 'SEED', seed), patch.object(
                ...
                token = server._WORKBENCH._seal_manual('cover', 3)
                self.assertEqual(verifier._unwrap_submission('cover', token), 3)
                self.assertIsNone(verifier._unwrap_submission('mul', token))
                self.assertIsNone(verifier._unwrap_submission('cover', '3'))
                with patch.object(verifier, 'SEED', seed + '-other-run'):
                    self.assertIsNone(verifier._unwrap_submission('cover', token))

    def test_no_default_key_can_prepare_a_submission(self):
        with patch.object(server._WORKBENCH, 'sealing_key', None):
            with self.assertRaisesRegex(RuntimeError, 'unavailable'):
                server._WORKBENCH._seal_manual('cover', 3)

    def test_key_fetch_uses_a_fixed_internal_path(self):
        with patch.object(server, 'VERIFIER_URL', 'http://verifier:18151/verify?extra=1'), patch.object(
            ...
            fetch.assert_called_once_with(
                'http://verifier:18151/workbench-key', timeout=server.REQUEST_TIMEOUT_SECONDS
            )

    def test_invalid_or_missing_key_aborts_bootstrap(self):
        invalid = [{}, [], {'key': None}, {'key': 'ab'}, {'key': 'zz' * 32}, {'key': ' ' * 64}]
        with patch.object(server, 'VERIFIER_URL', 'http://verifier:18151/verify'):
    ...
    def test_verify_proxy_cannot_select_a_different_internal_path(self):
        with patch.object(
            server, 'urlopen', return_value=io.BytesIO(b'{"checkpointId":"cover","correct":false}')
        ) as fetch:
            server.proxy_verdict(
                {'checkpointId': 'cover', 'submission': 'x', 'url': '/workbench-key'},
                'http://verifier:18151/verify',
            )
            self.assertEqual(fetch.call_args.args[0].full_url, 'http://verifier:18151/verify')
