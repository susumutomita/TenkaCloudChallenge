...
class Isolation(unittest.TestCase):
    ...
    def test_learner_cannot_read_seed_parent_or_network(self):
        source=(ROOT/'starter/clock_drill.py').read_text()+PROBE
        result=server._WORKBENCH.run_public_tests({'clock_drill.py':source})
    ...
    def test_failed_filter_does_not_execute_learner(self):
        source=(ROOT/'starter/clock_drill.py').read_text()+"\nprint('LEARNER_EXECUTED')\n"
...
@unittest.skipUnless(os.environ.get('CLOCK_WORKBENCH_URL'), 'requires the dedicated live Compose workbench')
class LiveProcessIsolation(unittest.TestCase):
    def call(self, path, payload=None):
        request = Request(os.environ['CLOCK_WORKBENCH_URL'] + path,
...
'''
        ...
        result = self.call('/api/test', {'files': {'clock_drill.py': starter['clock_drill.py'] + probe}})
    ...
    def test_private_key_route_is_not_a_public_proxy_route(self):
        for path in ('/workbench-key', '/api/workbench-key', '/verify/workbench-key'):
            for payload in (None, {'checkpointId': 'cover', 'submission': 'x'}):
