"""Hidden access to the ten clock drill lines' values.
...
def expected_for(seed: str) -> dict[str, object]:
    ...
    Two of the ten (`wrap` and `same-diff`) are ungraded — the platform's per-problem
    checkpoint maximum is eight — but `tests/hidden/check_clock_drill.py` still
