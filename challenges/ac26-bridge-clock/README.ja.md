...
nは5〜9で、合成数も含みます。原文候補と覆いの一対一対応は加法の性質です。確率の主張には、各メッセージの覆いを全0..n−1から一様に、すべての原文とも他の覆いとも独立に選び、観測者には伏せ、1通に使う条件が必要です。固定fixtureはその無作為な実験を実証するものではありません。同じ覆いの2観測から分かるのは差の余りであり、通常の符号付き差との等式ではありません。
