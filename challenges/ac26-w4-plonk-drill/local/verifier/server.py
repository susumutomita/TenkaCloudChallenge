...
Security contract:
  ...
  - No learner code is ever executed here. This problem is a drill: every checkpoint
    is a direct answer calculated on paper or with the optional Portal scratchpad. The grader for every checkpoint is `_check_line`: the pasted value,
...
This problem's `verifier/expected.py` computes seven expected values and checks the final construction from
the private fixture generator's public numbers. That module therefore ships only in this verifier image (and the author
...
#: prints the learner's own values; grading checks the eight pasted values.
CHECKPOINTS = GRADED
...
def public_payload(seed: str) -> dict[str, object]:
    ...
    The Portal Inspect assignments and the same numbers as a dict, with no answer
    derivation or private fixture material. Only the public values travel; expected values remain in the verifier.
    """
