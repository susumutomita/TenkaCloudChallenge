...
def main():
    ...
    if not rows:
        ...
        return 1
    passed=True
    print('== small example: p=7, a0=1,b0=2,g=1,w=6,tags=(1,2,3),beta=1,gamma=0 ==')
    for row in rows:
        try:
            got=call_row(drill,row,EXAMPLE)
            if row=='miss-count':
                same=construction_valid(got,EXAMPLE)
                print(f'{"PASS" if same else "FAIL"} {row}: got {got!r}; checks both broken wires, valid multiplication, equal nonzero products')
                passed &= same
                continue
