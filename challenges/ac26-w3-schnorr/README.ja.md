...
どれも正常系には現れません。sign して verify すれば、毎回通ります。この問題の 16 個の mutation の
...
`make reference-test` が mutation suite を実行します。壊した実装16種類があり、5種類は sign と verify
...
8項目を3段ヒントに変更しました。参加者向け資料だけの独立読解で、符号化API不足、ハッシュ衝突の誤説明、入力条件の曖昧さ、集合に見える辞書例を確認し、日英本文とstarterの説明を修正しました。作者は既存referenceでchallengeを余りに直す契約も確認しましたが、この内部確認は参加者プレーの証拠ではありません。カタログ検証は通過し、この文章修正で実Portalプレーは未実施です。


レビュー追補で公開テストに符号化の往復を追加し、採点器に秘密鍵・nonce・responseの不正入力を追加しました。参照実装で公開4件が通り、変異13種を拒否しました。作者の実行結果であり、参加者プレーの証拠ではありません。

追加レビューでは、署名の形の不備と、単位元・別の曲線・曲線外のコミットメントを拒否する採点を検証しました。参照実装と全16種類のmutation検査が通過。作者側の採点検証であり、実プレーの証拠ではありません。
