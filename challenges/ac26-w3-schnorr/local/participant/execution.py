    ...
    def call(self, module, function, args):
        ...
        while True:
            ...
            if isinstance(value, dict) and value.get('callId') == self.sequence:
                if 'error' in value:
                    raise {'InvalidKey': InvalidKey, 'InvalidEncoding': InvalidEncoding}.get(value['error'], LearnerError)('The submitted function raised an error.')
                if 'value' in value:
                    return decode(value['value'])  # inert values; only the parent applies the rules
            self._log_line(line)

    def module(self):
        return SimpleNamespace(InvalidKey=InvalidKey, InvalidEncoding=InvalidEncoding,
            **{name: (lambda *args, _name=name: self.call('schnorr', _name, args))
                for name in ('public_key', 'validate_public_key', 'commit', 'respond', 'verify_transcript', 'encode_point', 'decode_point', 'challenge_preimage', 'challenge', 'sign', 'verify', 'cross_protocol_witness')})
