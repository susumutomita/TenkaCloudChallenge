...
fifteen mutations are exactly that shape, which is why the checkpoints go after it directly.
...
`make reference-test` runs the mutation suite: fifteen broken implementations. Five of them sign and
...
Additional review validation rejects malformed signature shapes and identity, foreign-curve, and off-curve commitments. The reference and all fifteen mutation checks pass. These are author-side grading checks, not live participant-play evidence.
