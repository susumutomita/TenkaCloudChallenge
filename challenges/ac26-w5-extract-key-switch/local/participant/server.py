...
read the assertions. B2 took `fixtures/` out of this stage as well: it implements
`phase_coefficient`, `extract_sample`, `extract_trace`, `decompose_mask`, `key_switch` and
`domain_report` -- the six names `starter/extract.py` asks the learner to write -- because
it cannot derive this deployment's extraction trace, switched sample or domain report
without them. `show.py` and the public tests read this deployment's public half from the
verifier's `GET /public` instead (see show.py, tests/public/test_extract.py, and the
VERIFIER_PUBLIC_URL wiring in ../docker-compose.yml). The supplied half a participant still
needs stayed here, in `participant/ring.py`.
"""
