...
class LearnerSession:
    ...
    def module(self):
        return SimpleNamespace(**{name: (lambda *args, _name=name: self.call('extract', _name, args))
                                  for name in ('phase_coefficient', 'extract_sample', 'extract_trace', 'decompose_mask', 'key_switch', 'domain_report', 'extraction_counterexample')})
