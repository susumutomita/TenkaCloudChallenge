...
the Participant Portal -- so `tests/hidden/check_extract.py` (which every one of the eight
checkpoints is graded by) and `fixtures/generate.py` (which implements all six graded
functions, because it cannot derive this deployment's trace, switched sample or domain
report without them) shipped in the image a learner's own `make build` produced. Both now
live only here and in the `author` stage (see ../Dockerfile). The Portal moved to
`participant/server.py`, and `GET /public` below is what that image reads instead of
...
def _unwrap_submission(checkpoint_id: str, submission: object) -> object:
    ...
    ac26-w5-cmux-blind-rotation's verifier, for the same reason.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler's API
        ...
        if path == "/public":
            ...
            # always printed, plus the parameters, secrets, accumulator and switching key
            ...
            # where `show.py` and `tests/public/test_extract.py` get them from.
