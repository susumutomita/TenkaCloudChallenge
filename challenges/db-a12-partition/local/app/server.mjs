/**
 * db-a12-partition — local-play container entrypoint (Node half).
 *
 ...
 *          confirms the drill is up and shows live partition catalog state.
 ...
 * server.mjs) — metrics.events starts from a FIXED seed (local/db/seed.sql),
 * so the "correct end state" is a compile-time constant. See
 * local/grader/grade.mjs for the actual checkpoint logic.
 */
import { createServer } from "node:http";
import postgres from "postgres";
import { evaluateCheckpoint, isKnownCheckpoint } from "../grader/grade.mjs";
import {
  BYSTANDER_PARTITIONS,
  createPgGraderClient,
  DELETE_TARGET_PARTITION,
  DETACH_TARGET_PARTITION,
} from "./pg-client.mjs";
...
<title>db-a12-partition</title></head>
<body style="font-family:system-ui;max-width:34rem;margin:3rem auto;line-height:1.6">
<h1>大量削除の単位 ── row か partition か</h1>
<p>このドリルは画面の上では完結しません。Portal のターミナルを開き、以下で接続してください。</p>
<pre>psql -U participant -d drill</pre>
<p>ゴール: metrics.events の 2024-01 分の行 (${DELETE_TARGET_PARTITION}) を通常の
DELETE で消し、2024-02 分の partition (${DETACH_TARGET_PARTITION}) を
DETACH PARTITION (+任意で DROP TABLE) で消す。同じ 20,000 行規模の作業でも、
両者の所要時間がどれだけ違うかを自分の目で確認する。
2024-03〜06 (${BYSTANDER_PARTITIONS.join(", ")}) はこのドリルに一切関わらない
── 無傷のまま残さなければならない。</p>
...
const info = createServer(async (request, response) => {
  ...
  if (request.method === "GET" && url.pathname === "/status") {
    try {
      const [catalog, deleteTargetRows, detachTargetRows, bystanderRows] = await Promise.all([
        client.partitionCatalog(),
        client.deleteTargetRowCount(),
        client.detachTargetRowCount(),
        client.bystanderRowCounts(),
      ]);
      return send(response, 200, {
        deleteTarget: { partition: DELETE_TARGET_PARTITION, rows: deleteTargetRows, catalog: catalog[DELETE_TARGET_PARTITION] },
        detachTarget: { partition: DETACH_TARGET_PARTITION, rows: detachTargetRows, catalog: catalog[DETACH_TARGET_PARTITION] },
        bystanders: BYSTANDER_PARTITIONS.map((name) => ({ partition: name, rows: bystanderRows[name], catalog: catalog[name] })),
      });
    } catch {
