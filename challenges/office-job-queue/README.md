# Pass jobs between sites — SQS basics
...
Create the named Standard queue → one teammate sends → another receives and deletes → explain why receive is not completion.
...
One participant-created Standard SQS queue (SSE-SQS), checker and cleanup Lambdas, one-day log groups, built-in SQS metrics and a protected URL. No paid custom metric or alarm is created.
...
The checker requires positive AWS/SQS send, receive and delete metrics since creation AND zero visible/in-flight/delayed messages. Metrics are approximate and may take minutes; retry counts are not unique-message counts. The checker never sends, receives or deletes messages. A previously unused empty queue cannot pass. Use within six hours of queue creation, with four-day retention; no purge, deletion or attribute-update permission is granted to participants.
