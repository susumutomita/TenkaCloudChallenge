...
Workflow · **Role:** `diagnostic` · **Time:** 30–45 minutes · **Points:** 100
...
Six boxes, marked independently. Wrong answers cost 5 points each.
...
| `predict` | 15 | **you, by hand** | The number you end on, as one number — worked out **before** you run anything |
| `first-broken` | 15 | **you, by hand** | Which position holds the number that does not belong, counting from 0 at the left |
| `generalize` | 20 | Portal | Your whole `counter.py`, run against numbers you have not seen |
| `walkback` | 20 | **you, by hand** | From the second walk's final number, how many rounds it took (the recipe is in the statement) |
| `no-walkback` | 20 | **you, constructed** | Keeping that walk's step, one ring size on which no undo number exists (graded as a property) |

Hints are available on `first-broken` (7), `generalize` (5, 5), `walkback` (5, 5) and `no-walkback` (5, 5). Opening every hint
leaves 63 of the 100 points.
