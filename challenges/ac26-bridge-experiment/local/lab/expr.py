...
A rule is submitted as the expression that has to come out equal to the counter's
final value:

    (start + step*rounds) % modulus      ->  that expression, evaluated per case
    start % modulus                      ->  a rule that ignores the rounds

Grammar, the smallest thing that can express a closed form for this counter:
...
`%` is the remainder Python computes, which is the one this counter is defined by:
it always lands in [0, modulus) even when the left side is negative. That is not an
implementation detail to route around -- it is exactly the behaviour the backwards
counter in the transfer stage is asking about.

There is no division and no exponent. Neither is needed here, and both are ways for
a pasted line to turn into something that either takes a long time or means
something the participant did not intend.
"""
    ...
    while index < len(source):
        ...
        if char == "/":
            raise ExpressionError(
                "there is no division here: the counter is defined with `%`, the remainder, "
                "and `%` is available"
            )
        if char == "=":
            raise ExpressionError(
...
def join_arguments(arguments: list[str]) -> str:
    ...
    `counter rule (start + step*rounds) % modulus` without quotes arrives as several
    argv entries, and `*` may even have been globbed. Rejoining with spaces makes the
    unquoted form mean the same thing as the quoted one wherever the shell did not
    destroy it, which is one less way for a first terminal session to go wrong.
    """
