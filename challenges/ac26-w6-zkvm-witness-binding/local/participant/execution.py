...
Untrusted JSON function results and Env requests cross this boundary;
...
class LearnerSession:
    ...
    def call(self, function, args):
        ...
        environments = {index: arg for index, arg in enumerate(args) if isinstance(arg, Env)}
        wire_args = [['env', index] if index in environments else encode(arg)
                     for index, arg in enumerate(args)]
        next_operation, operation_bytes = 0, 0
        self._send({'callId': self.sequence, 'function': function, 'args': wire_args})
        while True:
            line = self._line()
            try:
                value = json.loads(line)
            ...
            if isinstance(value, dict) and value.get('callId') == self.sequence:
                if 'env' in value:
                    index = value['env']
                    if (type(index) is not int or index not in environments
                            or type(value.get('operationId')) is not int
                            or value['operationId'] != next_operation):
                        raise LearnerError('The input channel was malformed.')
                    operation_bytes += len(line.encode())
                    if operation_bytes > MAX_LOG_BYTES:
                        raise LearnerError('Input operations exceeded the limit.')
                    method = value.get('method')
                    if method not in ('public', 'variable', 'note', 'hint', 'write_private',
                                      'public_inputs', 'hints', 'read_private',
                                      'transcript', 'writes', 'reads'):
                        raise LearnerError('Unknown input operation.')
                    arguments, keywords = decode(value.get('args')), decode(value.get('kwargs'))
                    if not isinstance(arguments, (list, tuple)) or not isinstance(keywords, dict):
                        raise LearnerError('Malformed input arguments.')
                    reply = {'callId': self.sequence, 'operationId': next_operation}
                    try:
                        # The parent owns the actual writes/read counters and records.
                        # The child supplies actions, never a replacement transcript.
                        result = getattr(environments[index], method)(*arguments, **keywords)
                        reply['value'] = encode(result)
                ...
                if 'value' in value:
                    return decode(value['value'])  # untrusted values; parent checks them
            self._log_line(line)
    ...
    def module(self):
        ...
        return SimpleNamespace(**{name: function(name) for name in
                                  ('encode_statement', 'image_digest', 'guest_input',
                                   'run_guest', 'seal_journal', 'accept_receipt', 'leak_report')})
