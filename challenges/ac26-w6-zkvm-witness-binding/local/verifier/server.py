...
Security contract (docs/curricula/advanced-cryptography-2026/TEMPLATE.md §/verify):
  ...
  - The trusted process owns hidden checks. A fresh restricted worker receives only
    submitted source and function inputs, never a checker, fixture seed or verdict.
    Function values and Env operations cross a bounded channel; the parent computes
    the final decision and records all Env observations. The source tree is never written.
...
from fixtures.generate import public_payload  # noqa: E402
from tests.hidden import check_guest  # noqa: E402
from participant.execution import LearnerSession  # noqa: E402
...
def _run_submission(
    ...
    """Keep hidden checks in the parent; treat worker replies only as function values."""
    source = submission
    ...
    failures = []
    try:
        with LearnerSession({'guest.py': source}, timeout=RUN_TIMEOUT_SECONDS) as session:
            module = session.module()
            for name in phases:
                failures.extend(getattr(check_guest, name)(module, seed))
    except Exception:
        return False, 'The submitted functions did not complete the required input/output contract.'
