module rv32i_cpu_core #(
  ...
  parameter logic [31:0] JAL_LINK_OFFSET = 32'd4,
  parameter logic LB_SIGN_EXTEND = 1'b1
) (
  input  logic        clk,
  input  logic        reset_n,
    ...
    case (instr_rdata[6:0])
      ...
      7'b0110011: begin
        ...
        case (instr_rdata[14:12])
          ...
          3'b101: begin
            if (instr_rdata[30]) rd_value = $signed(rs1_value) >>> rs2_value[4:0];
            else rd_value = rs1_value >> rs2_value[4:0];
          end
  ...
  always_ff @(posedge clk) begin
    if (!reset_n) begin
      ...
      pc <= pc_next;
      if (rd_write && rd_index != 0) begin
        registers[rd_index] <= rd_value;
      end
      registers[0] <= 32'b0;
    end
  end
