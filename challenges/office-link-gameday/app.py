...
def scenario(lang):
    ...
    destinations = [{"id": str(i), "label": sites[i], "detail": codes[i]} for i in range(3)]
    return {"missions": [
        {
            ...
            "reasons": [
                {"id": "rename", "label": t("設定の名前を全部変える", "Rename every setting")},
                {"id": "region", "label": t("見ている地域をそろえる", "Look in the same region")},
                {"id": "more", "label": t("同じ設定を何個も作る", "Create several copies")},
            ],
        ...
        },
        {
            ...
            "fields": [
                {"id": "scope", "label": t("来場者に見せる資料", "Documents visitors may see"), "options": [{"id": "invitation", "label": t("案内だけ", "Invitation only")}, {"id": "all", "label": t("案内と社内名簿", "Invitation and staff list")}, {"id": "none", "label": t("何も見せない", "Nothing")}]},
                {"id": "permission", "label": t("許可する操作", "Allowed action"), "options": [{"id": "edit", "label": t("読む・書き換える", "Read and edit")}, {"id": "read", "label": t("読むだけ", "Read only")}]},
            ],
            ...
            "reasons": [{"id": "admin", "label": t("管理者権限があることだけ", "Only check administrator access")}, {"id": "both", "label": t("請求書は読める ＋ 給与表は読めない", "Invoices are readable + payroll is not")}, {"id": "block", "label": t("全部の資料が読めない", "No document can be read")}],
        ...
        },
        {
            ...
            "fields": [{"id": "backup", "label": t("どの控えへ戻す？", "Which copy should you restore?"), "options": [{"id": "early", "label": "10:05", "detail": t("赤いホール", "Red Hall")}, {"id": "good", "label": "10:15", "detail": t("青いホール", "Blue Hall")}, {"id": "late", "label": "10:25", "detail": t("案内なし", "No invitation")}] }],
            ...
            "lesson": t("青いホールの案内が戻りました。控えを残すだけでなく、戻して読めるか確かめて初めて復旧です。AWSのS3には、同じファイルの過去版を残す『バージョニング』という機能があります。", "The Blue Hall invitation is back. Keeping a copy is not enough: recover it and check that people can read it. AWS S3 offers versioning to retain earlier versions of a file."),
            "why": t("次の相談：控えの一覧に『保存成功』とあります。これだけで安心？", "Next request: a backup list says 'saved successfully'. Is that enough?"),
            "reasons": [{"id": "saved", "label": t("保存成功なら確認は不要", "Saving succeeded, so no check is needed")}, {"id": "latest", "label": t("一番新しい控えを無条件で使う", "Always use the newest copy")}, {"id": "rehearse", "label": t("戻して、中身が正しいか確かめる", "Restore and check the contents")}],
            "whyHint": t("『控えがある』と『必要な内容へ戻せる』は別です。使う人の目で確認します。", "Having a copy is different from recovering the content you need. Check it as a user."),
        },
    ]}
...
def evaluate(check, values, lang):
    ...
    if check.endswith("-why"):
        ...
        expected = {"delivery-why": "region", "sharing-why": "both", "restore-why": "rehearse"}[check]
    ...
    if check == "sharing":
        ok = values.get("scope") == "invitation" and values.get("permission") == "read"
    ...
    return values.get("backup") == "good", t("必要な案内へ戻っていません。消えた時刻と、会場変更の記録を見比べよう。", "The needed invitation is not restored. Compare the deletion time with the venue change.")
...
def handler(event, context=None):
    ...
    if method == "GET":
        ...
        return response(404, {"error": "not_found"})
    if method != "POST" or route != "api/play":
