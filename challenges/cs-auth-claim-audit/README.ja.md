...
監査対象の gateway は、署名検証という工程を正しく実装していた。それでも壊れている。

## 欠陥は 2 つ。どちらも動作テストでは見えない

**`alg` を token から読んでいる。** アルゴリズムは header にあり、header は token の中にあり、token は
いま本物か確かめようとしている対象そのものである。それを読むということは、**呼び出し側が自分の token の
検証方法を選べる**ということだ。`alg: "none"` を宣言した token の第 3 セグメントには、署名対象の本物の
SHA-256 が入っている。空でもなく、見た目に壊れてもおらず、通る。

「none を弾けばよい」では終わらない。これは 1 つの値についての事実ではなく、制御の向きの問題である。
後で別のアルゴリズムが増えれば、同じ穴がもう一度開く。

**`resource` を一度も見ていない。** `tenant` claim は「この token の持ち主が属する組織」を言う。
`resource["tenant"]` は「この文書が属する組織」を言う。この 2 つが一致することは、どちらの値からも
自動的には出てこない。**比較する行を書かないかぎり、比較は起きない**。そして書き忘れても、署名は通り、
期限も通り、scope も通り、テストも通る。

## 公開テストは starter を通す

これは事故でも、直すべき穴でもない。これが演習そのものである。

公開テストは署名を見て、期限を見て、scope を見て、壊れた入力を見る。書いた人がテナントのことを考えて
いなかったので、テナントについては訊かず、だから通る。緑が意味するのは 1 つだけ —
