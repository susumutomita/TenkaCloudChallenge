...
beaver.py against seeded settings; they differ in which hidden phases they run, and
...
`tests/hidden/check_beaver.py` shipped in the learner's own image alongside it, and every
checkpoint is graded by running that suite. `fixtures/generate.py` shipped there too, and
its `setting()` returns x, y, a, b and c in the clear. That Portal-facing surface now
lives in `participant/server.py`, in a separate image (see ../Dockerfile) that this
process's own container never builds; this file, `fixtures/` and `tests/hidden/` are
reachable only over the Compose-internal network (see ../docker-compose.yml), never from
the participant container's filesystem.
...
CODE_CHECKPOINTS = {
    ...
    "protocol": ("check_end_to_end", "check_rounds"),
    "transfer": (),
}
...
def _run_submission(
    ...
    if isinstance(source, dict):
        source = source.get("beaver.py")
    if not isinstance(source, str) or not source.strip():
        return False, None
    if len(source) > MAX_BODY_BYTES:
        return False, None
    try:
        with LearnerSession({'beaver.py': source}, timeout=RUN_TIMEOUT_SECONDS) as learner:
            module = learner.module()
            if phases:
                failures = []
                for name in phases:
                    failures.extend(getattr(check_beaver, name)(module, seed))
            else:
                failures = check_beaver.run(module, seed)
...
def main() -> None:
    protect_supervisor()
    port = int(os.environ.get("VERIFY_PORT", "18148"))
