...
def initialization_error(error, filename):
    ...
    if isinstance(error, SyntaxError) and error.filename == filename:
        ...
        while tb is not None:
            if tb.tb_frame.f_code.co_filename in ('beaver.py',):
...
def main():
    ...
    restrict_learner()
    modules = {name: types.ModuleType(name) for name in ('beaver',)}
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            # Enforce the published Python types before JSON could turn a tuple into
            # a list (or a bool into an integer). The parent still checks all values.
            if call['function'] in ('mask', 'combine'):
