"""Separated hidden verifier for the range-boundary report lab."""
...
from fixtures.generate import WINDOW_DAYS, daily_ledger, health_token, run_date
...
CHECKPOINTS = ("environment", "observe", "repair", "generalize")
...
PORTAL_ENGLISH_CONTRACT = {
    "name": "Last week's number counts a day that is not last week",
    "description": "Audit a daily report that calls itself the last 7 days, then count the days the contract actually names.",
    "labels": {
        "environment": "environment - paste the Workbench pass phrase",
        "observe": "observe - name the day that is not in the last seven, and the right total",
        "repair": "repair - count exactly the days the contract names",
        "generalize": "generalize - hold for any window length and across month and year ends",
    },
}
...
def _contract_total(seed: str) -> int:
    """Sum the ledger over the days the contract names, run date excluded.

    This lives here rather than in fixtures/generate.py on purpose: fixtures ship
    inside the participant image and this number is the answer to `observe`.
    """
    run = run_date(seed)
    counts = {row["date"]: int(row["count"]) for row in daily_ledger(seed)}
    return sum(
        counts[(run - timedelta(days=back)).isoformat()] for back in range(1, WINDOW_DAYS + 1)
    )
...
def _check_observe(submission: object) -> bool:
    ...
    if not isinstance(value, list) or len(value) != 2:
        return False
    day, total = value
    if not isinstance(day, str) or day.strip() != run_date(SEED).isoformat():
        return False
    # `type(...) is int` and not isinstance: True == 1 would otherwise grade as a total.
    return type(total) is int and total == _contract_total(SEED)
...
private_checker = getattr(check_report, {phase!r})
...
try:
    import report as submission_module
except Exception:
...
if not hasattr(submission_module, "report_total"):
    failures = ["submission does not define report_total()"]
else:
    try:
...
def _check_code(phase: str, submission: object) -> tuple[bool, str]:
    ...
    with tempfile.TemporaryDirectory() as workspace:
        ...
        Path(workspace, "report.py").write_text(submission, encoding="utf-8")
...
def _check_repair(submission: object) -> tuple[bool, str]:
    correct, detail = _check_code(CODE_CHECKPOINT_PHASES["repair"], submission)
...
def main() -> None:
    port = int(os.environ.get("VERIFY_PORT", "18591"))
