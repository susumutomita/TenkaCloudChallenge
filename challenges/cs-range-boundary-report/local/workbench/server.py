...
PORT = int(os.environ.get("WORKBENCH_PORT", "18590"))
VERIFIER_URL = os.environ.get("VERIFIER_URL", "http://verifier:18591/verify")

MAX_BODY_BYTES = 256 * 1024
MAX_OUTPUT_BYTES = 64 * 1024
RUN_TIMEOUT_SECONDS = 12
REQUEST_TIMEOUT_SECONDS = 15
SUBMISSION_FILES = ("report.py",)
CHECKPOINTS = ("environment", "observe", "repair", "generalize")
CODE_CHECKPOINTS = frozenset(("repair", "generalize"))
CHECKPOINT_LABELS = {
    "environment": "environment — Workbench の合言葉を貼る",
    "observe": "observe — 直近 7 日に入らない日と、正しい合計を答える",
    "repair": "repair — 契約どおりの 7 日だけを数える",
    "generalize": "generalize — 期間の長さや月・年をまたいでも同じ規則を保つ",
}
...
def config_payload() -> dict[str, object]:
    return {
        ...
        "name": "先週の数字に、先週じゃない日が入っている",
        "description": "「直近 7 日」を名乗る日次レポートを台帳と突き合わせ、数える範囲の端を契約どおりに直す。",
...
runpy.run_path({root!r} + "/tests/public/test_report.py", run_name="__main__")
"""
...
def run_public_tests(seed: str, files: object) -> dict[str, object]:
    ...
    if sources is None:
        return {"passed": False, "output": "report.py must be a non-empty Python file."}
    with tempfile.TemporaryDirectory() as workspace:
        Path(workspace, "report.py").write_text(sources["report.py"], encoding="utf-8")
...
def prepare_submissions(seed: str, files: object) -> dict[str, object]:
    sources = _submission_sources(files)
    if sources is None:
        return {"ok": False, "output": "report.py must be a non-empty Python file."}
    source = sources["report.py"]
    return {
        "ok": True,
        "submissions": {
