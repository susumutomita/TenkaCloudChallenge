...
if phases:
    failures = []
    for name in phases:
        failures.extend(getattr(check_sumcheck_drill, name)(sumcheck_drill, {seed!r}))
else:
    failures = check_sumcheck_drill.run(sumcheck_drill, {seed!r})
...
def _run_submission(submission: object, phases: tuple[str, ...], seed: str) -> bool:
    """Run the hidden suite against a `sumcheck_drill.py` in a throwaway workspace.
    ...
    if isinstance(source, dict):
        source = source.get("sumcheck_drill.py")
    if not isinstance(source, str) or not source.strip():
        return False
    if len(source) > MAX_BODY_BYTES:
        return False
    with tempfile.TemporaryDirectory() as workspace:
        (Path(workspace) / "sumcheck_drill.py").write_text(source, encoding="utf-8")
...
_WORKBENCH = PortalEditorSupport(
    ...
    problem_name='1 行打って、出た値を貼る — SumCheck を 2 ラウンド回して嘘を数える',
    problem_name_en='Type one line, paste the value — two rounds of SumCheck, then count the misses',
