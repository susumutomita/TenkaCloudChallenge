...
import sumcheck_drill as drill
from participant.exercise import ROW_ARGS,EXAMPLE,EXAMPLE_EXPECTED,call_row
from participant.model import valid_spoof,valid_pair
...
def main():
    ...
    if not rows:
        ...
        return 1
    passed=True
    print('== small example: p=5, x=[1,1,1,1] ==')
    for row in rows:
        try:
            got=call_row(drill,row,EXAMPLE)
            if row in ('lie-caught','miss-points'):
                same=(valid_spoof if row=='lie-caught' else valid_pair)(EXAMPLE,got)
