...
from fixtures.generate import GRADED,normalize_answer,setting,valid_spoof,valid_pair
    ...
    for context,inputs,expected in [('example',EXAMPLE,EXAMPLE_EXPECTED),('deployment',fixture['public'],fixture['expected'])]:
        for row in GRADED:
            try:
                ...
                valid=(valid_spoof if row=='lie-caught' else valid_pair)(inputs,actual) if row in ('lie-caught','miss-points') else actual is not None and actual==normalize_answer(row,expected[row])
                if not valid:
