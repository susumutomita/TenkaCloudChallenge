...
天下クラウド株式会社、 ネットワーク班に来て数週間。 前任の SRE は去り際に「IP 直書きはもうやめた。 これからは名前で繋ぐ」と言い残していった。 そのやり方で組み直された社内サービスが、 今朝から繋がらない。

> CTO 曰く: 「監視ダッシュボードが `core` を見失ってる。 踏み台 (`relay`) からは名前で呼んでるはずだ ── `core.internal.tenka.test` だったか。 名前は引けてる、 IP も返ってくる。 なのに繋がらない。 前任の SRE の置き土産が壊れてるのか、 そもそも嘘の名前なのか、 君が見極めてくれ」
