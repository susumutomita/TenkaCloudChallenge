...
PORT = int(os.environ.get("WORKBENCH_PORT", "18520"))
VERIFIER_URL = os.environ.get("VERIFIER_URL", "http://verifier:18521/verify")

MAX_BODY_BYTES = 256 * 1024
MAX_OUTPUT_BYTES = 64 * 1024
RUN_TIMEOUT_SECONDS = 12
REQUEST_TIMEOUT_SECONDS = 15
SUBMISSION_FILES = ("publish.py",)
CHECKPOINTS = ("environment", "observe", "audit", "publish", "durable", "generalize")
CODE_CHECKPOINTS = frozenset(("publish", "durable", "generalize"))
CHECKPOINT_LABELS = {
    "environment": "environment — Workbench の合言葉を貼る",
    "observe": "observe — 読み手が握っていたものを、対象ファイルと合わせて答える",
    "audit": "audit — 公開が不可分でなかった証拠になる観測を挙げる",
    "publish": "publish — 読み手に半端なファイルを一度も見せない",
    "durable": "durable — どの時点で落ちても丸ごと残し、作業ファイルを残さない",
    "generalize": "generalize — 新規名・繰り返し・公開の重なりでも成り立たせる",
}
...
def inspect_payload(seed: str) -> dict[str, object]:
    ...
    return {
        ...
        "audit": {
            "readerLog": [
                {"index": index, **row} for index, row in enumerate(observations)
            ],
...
runpy.run_path({root!r} + "/tests/public/test_publish.py", run_name="__main__")
"""
...
def run_public_tests(seed: str, files: object) -> dict[str, object]:
    ...
    if sources is None:
        return {"passed": False, "output": "publish.py must be a non-empty Python file."}
    with tempfile.TemporaryDirectory() as workspace:
        Path(workspace, "publish.py").write_text(sources["publish.py"], encoding="utf-8")
...
def prepare_submissions(seed: str, files: object) -> dict[str, object]:
    sources = _submission_sources(files)
    if sources is None:
        return {"ok": False, "output": "publish.py must be a non-empty Python file."}
    source = sources["publish.py"]
    return {
        "ok": True,
        "submissions": {
