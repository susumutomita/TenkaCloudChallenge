"""Separated hidden verifier for the atomic-publish lab."""
...
from fixtures.generate import health_token, published_document, reader_observations
...
CHECKPOINTS = ("environment", "observe", "audit", "publish", "durable", "generalize")
...
def _check_observe(submission: object) -> bool:
    expected = [published_document(SEED)["path"], "partial"]
...
def _check_audit(submission: object) -> bool:
    ...
    expected = [
        index
        for index, row in enumerate(reader_observations(SEED))
...
from tests.hidden import check_publish
private_checker = getattr(check_publish, {phase!r})
...
try:
    import publish as submission_module
except Exception:
...
if not hasattr(submission_module, "publish"):
    failures = ["submission does not define publish()"]
else:
    try:
...
def _check_code(phase: str, submission: object) -> tuple[bool, str]:
    ...
    with tempfile.TemporaryDirectory() as workspace:
        ...
        Path(workspace, "publish.py").write_text(submission, encoding="utf-8")
...
def _check_publish(submission: object) -> tuple[bool, str]:
    correct, detail = _check_code(CODE_CHECKPOINT_PHASES["publish"], submission)
    return correct and bool(SEED), detail


def _check_durable(submission: object) -> tuple[bool, str]:
    correct, detail = _check_code(CODE_CHECKPOINT_PHASES["durable"], submission)
    return correct and bool(SEED), detail
...
def main() -> None:
    port = int(os.environ.get("VERIFY_PORT", "18521"))
