...
`tests/hidden/check_rgsw.py` and the process that runs it. All eight checkpoints are graded
by running that suite against the submitted file, so the person being graded could read the
assertions. Option B2 took `fixtures/` out of this stage as well: it implements
`gadget_vector`, `decompose`, `recompose`, `decompose_poly`, `recompose_poly`,
`levels_needed`, `smallest_unrepresentable`, `rgsw_encrypt`, `external_product` and
`external_trace` -- the ten names `starter/rgsw.py` asks the learner to write -- because it
cannot derive a deployment's rows and traces without them. `show.py` and the public tests
read this deployment's public half from the verifier's `GET /public` instead (see show.py,
tests/public/test_rgsw.py, and the VERIFIER_PUBLIC_URL wiring in ../docker-compose.yml).
"""
...
PROBLEM_ID = "ac26-w5-rgsw-external"
SEED = os.environ.get("FLAG_SEED", "local-dev-seed")
PORT = int(os.environ.get("WORKBENCH_PORT", "18109"))
...
_WORKBENCH = PortalEditorSupport(
    ...
    seed=SEED,
    problem_id='ac26-w5-rgsw-external',
    problem_name='誰にも読めないビットを掛ける',
    problem_name_en='Multiply by a bit nobody can read',
    description='メッセージを載せた RLWE 暗号文と暗号化されたビットを掛ける。 selector 0 でゼロ、 1 でメッセージ、そして算術はどちらでも同一なので結果からどちらかは分からない。',
    description_en='Multiply an RLWE ciphertext by an encrypted bit. Selector 0 gives an encryption of zero, selector 1 gives the message back, and the arithmetic is identical either way — so the result says nothing about which it was.',
    checkpoint_labels={'decompose': '値を桁へ分解する', 'gadget': 'gadget vector を書く', 'polynomial': '係数ごとに分解する', 'rgsw': 'selector を暗号化する', 'external': 'ビットを掛ける', 'trace': '積み上がりを見せる', 'failure': 'level が足りなくなる点', 'transfer': '見たことのない設定で成立させる'},
    checkpoint_labels_en={'decompose': 'Break a value into digits', 'gadget': 'Write the gadget vector', 'polynomial': 'Decompose per coefficient', 'rgsw': 'Encrypt the selector', 'external': 'Multiply by the bit', 'trace': 'Show the accumulation', 'failure': 'Where the levels run out', 'transfer': 'Hold up in a setting you have not seen'},
    submitted_files=('rgsw.py',),
    code_checkpoints=('decompose', 'gadget', 'polynomial', 'rgsw', 'external', 'trace', 'failure', 'transfer'),
    checkpoints=('decompose', 'gadget', 'polynomial', 'rgsw', 'external', 'trace', 'failure', 'transfer'),
