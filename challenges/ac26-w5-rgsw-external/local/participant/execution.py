...
class LearnerSession:
    ...
    def module(self):
        def function(name):
            return lambda *args: self.call('rgsw', name, args)
        return SimpleNamespace(**{name: function(name) for name in ('gadget_vector', 'decompose', 'recompose', 'decompose_poly', 'recompose_poly', 'levels_needed', 'smallest_unrepresentable', 'rgsw_encrypt', 'external_product', 'external_trace')})
