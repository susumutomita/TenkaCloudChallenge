...
same Docker stage and process that also graded `no-communication` -- so what that
checkpoint compares against shipped in the one image a learner's own `make build`
produced, in `fixtures/generate.py`. Moving the comparison alone would not have closed
that: it is plain module data, so it was the answer regardless of where the `==` lived.
`fixtures/` is not copied into this stage at all any more (see ../Dockerfile), and this
deployment's public evidence -- the setting, the shares and the four operation names --
is fetched from the verifier's `GET /public` at runtime by `show.py` and
`tests/public/test_linear.py`, which is the only source for it here.
"""
...
PORT = int(os.environ.get("WORKBENCH_PORT", "18096"))
...
_WORKBENCH = PortalEditorSupport(
    ...
    code_checkpoints=('add-shares', 'add-constant', 'mul-constant', 'transfer'),
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802 - stdlib handler API
        ...
        # `no-communication` answer that never went through `/api/prepare` is rejected
        # even by a caller who bypasses this process entirely.
