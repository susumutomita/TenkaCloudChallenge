...
一覧は問題文の Python 補助欄。追加ファイル/ネットワーク読み込みは不可。計算は20秒以内。
...
比較x>yは、法7でyの2シェアを0にし、a0を0/1、a1を0/6に変えると、
比較結果の2×2表は [0,1] / [1,0]。手元の出力F(a0),G(a1)の合計なら
対角同士の和は並べ替えで等しいはずだが、表では0と2で違う。したがって通信が要る。
奇数の素数pでも6をp-1にすれば同じ理由が成り立つ。
