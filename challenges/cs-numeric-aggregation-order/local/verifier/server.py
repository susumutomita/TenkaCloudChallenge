"""Separated hidden verifier for the aggregation lab."""
...
from fixtures.generate import health_token, disputed_report, reconciliation_runs
...
def _check_observe(submission: object) -> bool:
    expected = [disputed_report(SEED)["reportId"], "order-dependent"]
...
def _check_audit(submission: object) -> bool:
    ...
    majority = max({row["reportedTotal"] for row in runs}, key=lambda total: sum(
        1 for row in runs if row["reportedTotal"] == total
    ))
    expected = [index for index, row in enumerate(runs) if row["reportedTotal"] != majority]
...
private_checker = getattr(check_aggregate, {phase!r})
...
try:
    import aggregate as submission_module
except Exception:
...
if not hasattr(submission_module, "summarize"):
    failures = ["submission does not define summarize()"]
else:
    try:
...
def _check_code(phase: str, submission: object) -> tuple[bool, str]:
    ...
    with tempfile.TemporaryDirectory() as workspace:
        ...
        Path(workspace, "aggregate.py").write_text(submission, encoding="utf-8")
...
def _check_total(submission: object) -> tuple[bool, str]:
    correct, detail = _check_code(CODE_CHECKPOINT_PHASES["total"], submission)
    return correct and bool(SEED), detail


def _check_allocate(submission: object) -> tuple[bool, str]:
    correct, detail = _check_code(CODE_CHECKPOINT_PHASES["allocate"], submission)
    return correct and bool(SEED), detail
...
def main() -> None:
    port = int(os.environ.get("VERIFY_PORT", "18531"))
