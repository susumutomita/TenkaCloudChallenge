...
import rotation_drill as drill
from participant.exercise import ROW_ARGS,EXAMPLE,EXAMPLE_EXPECTED,call_row
from participant.model import valid_window,valid_edge
...
def main():
    ...
    if not rows:
        ...
        return 1
    passed=True
    print('== small example: p=4, q=16, n=4 ==')
    for row in rows:
        try:
            got=call_row(drill,row,EXAMPLE)
            if row in ('window','edge'):
                same=(valid_window if row=='window' else valid_edge)(EXAMPLE,got)
