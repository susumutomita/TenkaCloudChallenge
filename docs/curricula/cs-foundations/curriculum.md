...
第 1 章と第 2 章まで実装済み。埋まっていないものを埋まったことにしないため、下の「これから作るもの」には
第 3 章以降の順序の根拠だけを書く。
...
- **第 3 章以降は未実装。** 第 2 章で snapshot と決定論的 schedule の語彙までは渡せたが、I/O 境界、
  cache、再送へ転用する問題はまだ無い。将来順序を、実装済みであるかのようにカタログへは出さない。
