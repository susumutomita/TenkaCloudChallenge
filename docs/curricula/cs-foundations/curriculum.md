...
以降は未実装。埋まっていないものを埋まったことにしないため、下の「これから作るもの」に順序の根拠だけを
書いておく。
...
2. **トランザクションと可視性**
   ...
   単体テストでも結合テストでも緑になり、同時実行が来たときだけ壊れる。監査対象は「一貫していない
   まま返された応答のログ」になる。
3. **並行性 (I/O 境界)**
...
- **章が 1 つしかない。** 上の 5 章構想のうち実装済みは 1 章だけで、track として「順序」を主張できる
  状態ではない。2 章が入るまで、この track の `order` は将来の並びを予約しているだけである。
