...
| order | 問題 | 難易度 | 章 |
| --- | --- | --- | --- |
| 2 | `stackstack-first-request` | d1 | 0. 安全に始める |
...
上の表に入っている `stackstack-*` 9 問は本編と同じ世界観の local play 版で、これらは
コンテナだけで完走できる。**9 問を終えた人が次に踏む一歩は「もう 1 問解く」ではなく「AWS
アカウントを用意する」**であり、ルートはそこで一度性質が変わる。

`battles/stackstack` の instructions は冒頭でそう宣言している。ルートの案内でそれを隠さない
こと — 隠すと、9 問を終えた人がカードを開いて「自チームに deploy されていません」で行き止まる。
...
ルートを未経験者に見せて得たフィードバックの要旨と、それぞれへの設計上の回答をここに固定する。回答の一部は実装済み、残りは「既知の断絶 6」として下に列挙する。

| フィードバックの要旨 | 設計上の回答 |
| --- | --- |
| AWS は大量課金の事故記事が目に入るので怖い。「安全に試せる」というコメントがあると安心 | 安全宣言をルート入口の参加者可視面に置く (実装済み)。下の「安全宣言」参照 |
| 未経験者には今の構成でも難しい。もっと基礎から問題を作らないといけない | 章 0「安全に始める」を新設。1 問目 `stackstack-first-request` (order 2) は実装済み、残り 2 問は「既知の断絶 6」 |
...
1 段階目は**既に真である事実を、参加者から見える場所に書く**こと。ルート上の問題はすべて手元のコンテナだけで動き、クラウドアカウントも課金も発生せず、壊しても環境を作り直せば元に戻る。この事実は問題リポジトリの README には書かれていたが、参加者が最初に見るポータルのカードと instructions には書かれていなかった。安全宣言は `stackstack-onboarding` の `shortDescription` と `instructions` (ja / en) に置き、章 0 の新設でルート先頭になった `stackstack-first-request` も同じ宣言を参加者可視面に持つ。
...
上の「`stackstack-*` 9 問はコンテナだけで完走できる」は 9 問についての記述であって、ルート
...
### 6. 章 0 がまだ 1 問しかない (2026-08 勉強会フィードバック)

`stackstack-onboarding` (d1) はポータル・アプリ・ログ・設定・採点を 15 分で一周させるが、「Web アプリとは何か」「リクエストとレスポンスの往復」「コンテナはどこで動いていて、何にお金がかかるのか」を暗黙の前提にしている。勉強会のフィードバックは、未経験者にはこの前提こそが壁だと指摘した。

対応は章 0 (order 10 より手前) の問題群。1 問ずつ独立にレビューできるよう、新規問題は別 PR で追加する。選定基準は「ファネル」の**「出す前に最低限これだけ」** — バイブコーダーが自分のプロダクトを安全に出すために要る最小集合だけを置き、可用性・安定性の作り込みはオーバーキルなので本編以降に置く。
...
残り 2 つは空席のままで、order 4 / 6 を確保してある。
