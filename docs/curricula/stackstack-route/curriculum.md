...
| AWS を使わなければ敷居は下がる (Cloudflare 無料枠など)。ターゲット次第 | ターゲットは変えない。終点は AWS 上の StackStack 本編。ただし表の 15 問は全部 local play で AWS 不要であり (learning-path テストが固定済み)、「AWS が要るのは終点だけ」を導線で隠さない |
...
対応は章 0 (order 10 より手前) の新規問題群。新規問題には human play evidence が要る (`docs/PLAYABILITY_GOVERNANCE.md`) ため、作成はこの PR に混ぜず別 PR で行う。案:
