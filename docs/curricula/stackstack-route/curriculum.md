...
対応は章 0 (order 10 より手前) の新規問題群。新規問題には human play evidence が要る (`docs/PLAYABILITY_GOVERNANCE.md`) ため、作成はこの PR に混ぜず別 PR で行う。選定基準は「ファネル」の**「出す前に最低限これだけ」** — バイブコーダーが自分のプロダクトを安全に出すために要る最小集合だけを置き、可用性・安定性の作り込みはオーバーキルなので本編以降に置く。案:
