...
`wp2shell-friday-night-patch` は runtime を宣言しておらず local play で起動できないため、ルートから外した。導線に入れると解けない問題へ誘導することになる (Issue 402 と同じ問題)。結果として 5 章は `wp2shell-local-lab` の 1 問だけになる。

`wp2shell-local-lab` 自身が「本番を触る前の予行演習」という位置づけなので、**予行演習だけあって本番が無い**状態になっている。402 の解消後に再検討する。
