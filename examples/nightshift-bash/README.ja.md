...
Bash Battleの自主練教材 v0.1.0。**調べる → 無害に実証する → 業務を止めずに直す → 再起動で確かめる**、3ミッションの実践演習です。
...
**この版は単体CLI版です。TenkaCloudに登録・統合済みではありません。2026-09-27にmacOS/ColimaのDocker 29.6.1でビルドと統合テスト56項目が成功し、模範修復の1,000点を確認しました。最新の検証範囲は [検証記録](docs/PR-VERIFICATION.md) にあります。**
...
**順位を付けない自主練・ワークショップ用です。** 模範解答はこの公開リポジトリから誰でも読めます。0〜1,000点は自分の修復状況を確かめる目安であり、競技得点ではありません。参加者の順位・賞品・採用や認定の評価には使わないでください。コンテナから解答を除いても、解答が秘密になるわけではありません。
...
├── host/check-state.sh        ホストから実行する状態検証器
...
ただし、コンテナを任意の悪意あるコードに対する絶対的な隔離と扱わないでください。外部参加者に渡すワークショップでは専用VMや使い捨てホストを使い、事前リハーサルをしてください。セキュリティ境界と未検証事項は `docs/SECURITY.md` にあります。
