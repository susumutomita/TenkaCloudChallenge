...
- **A service that publishes ports must also join a non-internal network.** Docker establishes *no* host port binding for a container attached only to `internal: true` networks — it records the request and silently drops it, so `docker ps` shows a bare `8080/tcp` and the platform's `/verify` probe times out while the in-container healthcheck still reports `Up (healthy)`. If your lab needs a no-egress network, keep it and add a second bridge with `com.docker.network.bridge.enable_ip_masquerade: "false"` (and `host_binding_ipv4: "127.0.0.1"`): it carries the inbound published path and still has no outbound return route. Worked examples: `challenges/ai-riscv-screen-repair`, `challenges/secure-ota-rollback`. Enforced by `scripts/local-play-publish-reachability.test.ts`.
- **Run the container unprivileged.** Every local-play image ends with a `USER` (`node`, `lab`, `postgres`, …). This is not only hygiene: `initdb` refuses to run as uid 0, so a Postgres-based image left as root exits(1) on its first boot before anything listens.
...
- **`scoring.checks[]`** — 2–8 checks (4–6 recommended). Each check is `{ "id": "<slug>", "label": "<player-facing>", "input"?: "text" | "multiline", "points": <int>, "wrongAnswerPenalty"?: <int>, "hints"?: [...] }`. Use `input: "multiline"` when the participant submits source code or any value whose newlines must survive Portal entry; omitted / `text` retains the one-line input for answers and tokens. `id` matches `^[a-z0-9][a-z0-9-]{0,63}$` and is unique; `label` is ≤80 chars; `wrongAnswerPenalty` (if set) is ≤ that check's `points`; each check's hint penalties sum to ≤50% of that check's points; hint `id`s are unique **across the whole problem** (the portal reveal route is keyed on `hintId` alone). The `checks[].points` **sum** is the problem total and must equal the tier standard (§14). Points are the platform's source of truth — a `points` field in the container verdict is ignored for `multi-verify`.
