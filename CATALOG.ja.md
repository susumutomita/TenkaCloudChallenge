...
| キー             | 用途                                                                                                 |
| ---------------- | ---------------------------------------------------------------------------------------------------- |
| `instructions`   | 競技者の問題ページに**常時表示**するネタバレ禁止の「最初の一手・進め方」(#1929)。 fairness contract (#1124) で portal に出さない `description` と違い、 これは常に出る。 platform 側は Markdown 描画なので番号付き手順・コードブロック・画像 (アーキ図、 http/https `src`) が使える。 採点数値 / hardened state / surprise はここに書かない。 |
| `i18n.en`        | 英語訳 (`name` / `shortDescription` / `description` / `instructions` / `learningGoals`)。 ja は top-level、 en はここに。 サポート locale は **ja + en のみ** (#1108)。 |
