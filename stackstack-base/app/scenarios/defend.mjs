...
async function runRound(round) {
  ...
  // --- normal: the moderator publishes something they do not own.
  const publishable = drafts.filter(
    (draft) => draft.ownerId !== "u-mod" && draft.id !== MARKER_DRAFT_ID,
  );
  if (publishable.length > 0) {
    const target = pick(publishable, round, "pub");
    const result = await call("POST", `/api/publish?id=${target.id}`, actorById.get("u-mod"));
    if (result === null) {
      counters.transportErrors += 1;
    } else {
      counters.normal.publishTotal += 1;
      const id = result.body?.publication?.publicationId;
      // Not "it answered 201": the publication has to be in the queue
      // afterwards, so a route that says yes and drops the work on the floor is
      // not credited.
      const landed =
        result.status === 201 &&
        result.body?.publication?.draftId === target.id &&
        published.some((entry) => entry.publicationId === id);
      if (landed) counters.normal.publishServed += 1;
      else if (result.status === 201) {
        fail("normal-publish", "承認役の公開が受け付けられたのに、 公開キューに載っていない");
      } else fail("normal-publish", `承認役の公開が ${result.status} で止まった`);
    }
  }
  ...
  // --- attack: publishes the rule forbids — somebody else's draft, and the
  //     moderator's own (the 4-eyes half people forget).
  const attackPublishes = [];
  const nonModerators = ACTORS.filter((actor) => actor.role !== "moderator");
  const notOwnByOther = drafts.filter((draft) => draft.ownerId !== "u-mod");
  if (notOwnByOther.length > 0) {
    const actor = pick(nonModerators, round, "apA");
    const targets = notOwnByOther.filter((draft) => draft.ownerId !== actor.id);
    if (targets.length > 0) attackPublishes.push({ actor, draft: pick(targets, round, "apB") });
  }
  const moderatorOwn = drafts.filter((draft) => draft.ownerId === "u-mod");
  if (moderatorOwn.length > 0) {
    attackPublishes.push({ actor: actorById.get("u-mod"), draft: pick(moderatorOwn, round, "apC") });
  }
