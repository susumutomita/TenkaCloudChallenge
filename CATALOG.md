...
## AWS certification drill set

105 self-paced `flag` Challenges map to **every active AWS certification** (Issue #45). Each is a free-tier ($0) CloudFormation lab whose flag must be **derived by analyzing the deployed configuration** — the answer lives only in a CFn Output the participant role cannot read, so points come from reasoning, not from `aws ssm get-parameter`. Full per-exam listing: [`CERTIFICATION-INDEX.md`](./CERTIFICATION-INDEX.md) (generated by `scripts/build-cert-index.ts`).

| Tier | Certification | Problems |
| --- | --- | --- |
...
│   ├── iam-least-privilege/
│   └── <clf-*|saa-*|...>/         # 105 AWS-certification drill labs (see CERTIFICATION-INDEX.md)
